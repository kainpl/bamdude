 ...
 * (e.g. BamDude on 192.168.1.0/24, printer on 10.1.1.0/24) couldn't
...
describe('AddPrinterModal — custom subnet (#1564)', () => {
  ...
  beforeEach(() => {
    scanCalls = [];
    ssdpStarted = false;
    // Our test env uses jsdom's real localStorage (see setup.ts), so clear
    // persisted state and spy on setItem rather than mocking the whole API.
    localStorage.clear();
  ...
  it('routes a custom CIDR through startSubnetScan, not SSDP', async () => {
    ...
    expect(setItemSpy).toHaveBeenCalledWith(
      'bamdude.discovery.customSubnet',
      '10.1.1.0/24',
    );
  });
