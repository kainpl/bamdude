...
label matches what the operator sees on the Inventory page. When
``display_name`` is missing (Spoolman path; programmatic API callers), the
backend composes a fallback name as ``color_name → slicer_filament_name →
"{brand} {material}"``.
"""
...
class LabelSpoolEntry(BaseModel):
    ...
    label's bold central line — when provided it replaces the backend-side
    fallback (color_name → slicer_filament_name → "{brand} {material}").
    Frontend posts the value composed by ``formatSpoolDisplayName`` so the
    label matches the user's Inventory naming-template setting.
    """
...
def _spool_to_label_data(spool: Spool, deeplink_base: str, display_name: str | None) -> LabelData:
    """Local-DB spool → LabelData.

    ``display_name`` wins when provided (frontend's template-composed name);
    otherwise the same fallback chain upstream uses keeps the label
    self-sufficient for programmatic callers.
    """
    ...
    return LabelData(
        ...
        # Local Spool model has no storage_location column today; reserved for
        # future schema growth without breaking the renderer signature.
        storage_location=None,
        deeplink_url=f"{deeplink_base}/inventory?spool={spool.id}",
    )


def _spoolman_dict_to_label_data(s: dict, deeplink_base: str, display_name: str | None) -> LabelData:
    ...
    embedded filament. Material and brand come from filament/vendor. The
    optional ``display_name`` override still wins so a frontend that wants to
    apply BamDude's naming template against Spoolman data can.
    """
    ...
    name = (display_name or "").strip() or fname or material or "Spool"
...
async def render_local_inventory_labels(
    ...
    data_list = [_spool_to_label_data(s, deeplink_base, name_overrides.get(s.id)) for s in ordered]

    pdf = render_labels(body.template, data_list)
    filename = f"bamdude-labels-{body.template}.pdf"
...
async def render_spoolman_labels(
    ...
    deeplink_base = await _resolve_deeplink_base(request, db)
    data_list = [
        _spoolman_dict_to_label_data(by_id[sid], deeplink_base, name_overrides.get(sid)) for sid in requested_ids
    ]

    pdf = render_labels(body.template, data_list)
    filename = f"bamdude-labels-spoolman-{body.template}.pdf"
    return _stream_pdf(pdf, filename)
