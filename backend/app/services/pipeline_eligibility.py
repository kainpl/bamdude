"""Eligibility matcher for Slicer Pipeline runs (#1425, ported to BamDude).

Given a pipeline + the user's target (a pinned printer or a printer-model
class), this returns a structured report of issues the operator should resolve
before running. The frontend displays the report; the user can ``Run anyway``
to proceed (lenient policy — the print may still fail at the printer, but we
don't refuse the click).
...
Issue kinds (pinned for tests + i18n keys):
  ...
  - printer_disabled        — Printer.is_active is False
  ...
  - filament_unverified     — pipeline filament preset is a non-local tier we
                              ...
                              double-check
  - no_class_matches        — target_kind='printer_class' with zero matching printers
  - class_not_set           — target_kind='printer_class' with no target_model_class
...
AMS shape (BamDude): ``raw_data["ams"][].tray[].{tray_type,tray_color}`` for AMS
slots + ``raw_data["vt_tray"][]`` for the external spool — same field names the
print scheduler reads.
"""
...
# Same equivalence map as the print scheduler's canonical-filament-type helper
# but kept local so this module has no upward dependency on the scheduler.
# Mirrors the scheduler's behaviour: BBL-prefixed product names normalise to the
# base type (e.g. "PLA Basic" → "PLA"). When the scheduler's map gets a new
# alias, this one needs the same entry.
_FILAMENT_EQUIV_MAP = {
    "PLA": "PLA",
    "PLA BASIC": "PLA",
    "PLA MATTE": "PLA",
...
def _ams_slots(raw_data: dict) -> list[tuple[str, str]]:
    ...
    order. Uses the same field shape as the print scheduler's required-filament
    check.
    """
...
async def check_pipeline_eligibility(
    ...
    *,
    ...
    """
    # A ``target_printer_id`` being set is the source of truth for the
    # specific-printer path even when target_kind still holds the PR A default.
    if pipeline.target_printer_id is not None or pipeline.target_kind == "specific_printer":
        # Specific-printer branch.
