...
export function SpoolFormModal({
  ...
  // Structured storage-location catalog (upstream #1505).
  ...
  const bulkCreateMutation = useMutation<
    ...
    Error,
    { data: Record<string, unknown>; qty: number; autoIncLot: boolean }
  >({
    mutationFn: ({ data, qty, autoIncLot }) =>
      spoolmanMode
    ...
    onSuccess: async (result) => {
      ...
      const spoolmanResult = (spoolmanMode && !Array.isArray(result) && 'created' in result)
  ...
  const handleSubmit = () => {
    ...
    // Backend derives the denormalized storage_location string; omitting on an
    // untouched edit avoids stale overwrites of the Spoolman location field.
