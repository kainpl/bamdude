...
# tearing down before proceeding anyway (upstream #2521). Teardown is normally
# sub-second (cancel pump + close socket); the cap only guards a wedged upstream
# close.
_TEARDOWN_WAIT_SECONDS = 10.0

# How often a subscriber that isn't receiving frames re-checks whether its client
# is still connected. Only pays a cost when the stream is *not* producing frames
# — the normal path returns from queue.get() as soon as a frame lands and checks
# after the yield. Kept short because the subscriber count derived from it is
# what /camera/stop uses to decide whether to tear the upstream down (#2521).
...
class MjpegBroadcaster:
    ...
    def __init__(self, key: str, factory: UpstreamFactory, predecessor: MjpegBroadcaster | None = None) -> None:
        ...
        # cams where that wait looked like a permanent black screen, because the
        # browser never fired onLoad and the stall detector reconnect-looped
        # (upstream #2521).
        self._last_chunk: bytes | None = None
        # Set once teardown is fully complete (pump cancelled AND the upstream
        # socket closed). A successor waits on this before dialing so a
        # single-connection printer never sees two sockets at once — the overlap
        # stranded frames on an orphaned socket for the ~20 min it took the
        # printer's TCP keepalive to reap it (upstream #2521).
        ...
        # get_or_create) keeps it correct when concurrent viewers race to replace
        # the same stopped broadcaster — only the single pump dials.
    ...
    async def subscribe(self) -> asyncio.Queue[bytes]:
        ...
        async with self._lock:
            ...
            # (upstream #2521). The very first subscriber has nothing to prime
            # yet — it starts the pump below.
    ...
    async def force_shutdown(self) -> None:
        ...
        # Upstream socket is now closed (the pump's finally ran) — release anyone
        # waiting to open a replacement broadcaster (upstream #2521).
    ...
    async def wait_until_torn_down(self) -> None:
        ...
        old and new upstream sockets never overlap (upstream #2521).
        """
...
async def get_or_create_broadcaster(key: str, factory: UpstreamFactory) -> MjpegBroadcaster:
    ...
    async with _registry_lock:
        ...
        # behind its socket close so a single-connection cam never sees two
        # sockets at once (#2521).
...
async def shutdown_broadcaster(key: str) -> bool:
    ...
    popped *before* ``force_shutdown()`` was awaited, which vacated the slot while
    the upstream socket was still closing: a ``/camera/stream`` request landing in
    that window found nothing, minted a broadcaster with ``predecessor=None`` and
    dialled the printer immediately. That is exactly the two-sockets-at-once
    overlap the predecessor gate exists to prevent — the gate only engages when
    the stopped broadcaster is still *findable*, so popping it here bypassed the
    gate in the one case it was written for. A page reload fires ``/camera/stop``
    and the new stream request concurrently, so a single-connection cam
    (chamber-image port 6000) ended up with an orphaned socket the printer kept
    feeding, starving the live viewer until its TCP keepalive reaped it ~20 min
    later (upstream #2521).
    ...
    most one entry per printer, and the next viewer overwrites it.
    """
    async with _registry_lock:
    ...
    try:
        while True:
            try:
                ...
                # subscriber for up to half a minute — and ``/camera/stop`` trusts
                # that count to decide whether to tear the upstream down, so a
                # phantom subscriber could make it skip teardown entirely
