...
# ⚠️ Gitea clamps ``per_page`` to MAX_RESPONSE_ITEMS, which defaults to 50 —
# regardless of what we asked for. Consulted only when a tree response carries
# no usable ``total_count``: a page at least this long may be a clamped FULL
# page and so cannot be assumed to be the last one.
...
class GiteaBackend(GitHubBackend):
    ...
    async def _blob_shas_at(
        ...
        """Paged override of GitHub's single-GET tree read.
