...
- Hashed at rest (pbkdf2_sha256). Ephemeral tokens are stored as raw strings
...
class LongLivedToken(Base):
    ...
    # verify() can fetch one row instead of scanning + hashing all rows.
    # Format: ``bblt_<8-char-prefix>_<32-char-secret>``.
    lookup_prefix: Mapped[str] = mapped_column(String(8), nullable=False, index=True)
    # pbkdf2_sha256 hash of the full token. Never stored or returned in plaintext.
