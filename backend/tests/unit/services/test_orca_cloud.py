"""Tests for the Orca Cloud device-pairing service â€” device-code request,
...
class TestRequestDeviceCode:
    ...
    async def test_sends_client_id_scope_and_user_agent(self, svc):
        ...
        assert kwargs["headers"]["User-Agent"].startswith("BamDude/")
    ...
    async def test_invalid_client_raises_auth_error(self, svc):
        """A wrong/unregistered client_id returns ``invalid_client`` â€” an
...
class TestPollToken:
    ...
    async def test_rfc_error_codes_map_to_statuses(self, svc, error_code, expected):
        ...
        control flow â€” returned as statuses, never raised."""
    ...
    async def test_unknown_error_raises(self, svc):
        """An unrecognized error body is a real problem, not a poll state â€”
...
class TestRefresh:
    ...
    async def test_rotates_refresh_token(self, svc):
        """Refresh tokens are single-use â€” every successful refresh returns a
    ...
    async def test_rejected_refresh_clears_tokens(self, svc):
        ...
        unrecoverable â€” clear the stale credentials so the UI flips to
...
class TestApiHeaders:
    def test_api_headers_include_bearer_and_ua_no_apikey(self, svc):
        """External API auth is a plain bearer token â€” the old Supabase
...
class TestListProfiles:
    ...
    async def test_pull_hits_external_path_without_cursor(self, svc):
        ...
        with no ``?cursor=`` â€” ``cursor=0`` trips ``410 cursor_too_old``."""
        svc.access_token = "oc_ext_A"
        svc.token_expiry = datetime.now(timezone.utc) + timedelta(hours=12)
