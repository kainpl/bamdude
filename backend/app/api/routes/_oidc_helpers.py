...
Hosts the SSRF guard for admin-supplied icon URLs. Stricter than the
Spoolman SSRF guard (which intentionally allows loopback / RFC-1918
because same-LAN topology is the standard Spoolman deployment) — OIDC
icons must be reachable on the public internet (IdP-hosted), so private
addresses there are SSRF probes, not legitimate use cases.
...
def assert_safe_public_https_url(url: str) -> None:
    ...
    Used for OIDC provider icon URLs (#1333). Stricter than the Spoolman
    SSRF guard: also rejects loopback, private (RFC-1918), and link-local
    addresses because an OIDC icon legitimately lives only on the public
    internet.
    ...
    - Numeric-encoded IPv4 (decimal ``2130706433``, hex ``0x7f000001``) is
      ...
      Python's ``ipaddress`` raises ``ValueError``, so they bypass the
      IP-class block below if not caught first.
    - Cloud-provider metadata endpoints (``169.254.169.254``,
      ``100.100.100.200``, ``fd00:ec2::254``) — classic SSRF credential-
      exfil targets.
    - Loopback (``127.0.0.0/8``, ``::1``), private RFC-1918 (10/8,
      172.16/12, 192.168/16) and link-local (169.254/16, fe80::/10).
    - Multicast (224.0.0.0/4, ff00::/8) and unspecified (0.0.0.0, ::).
    - IPv4-mapped IPv6 (``::ffff:127.0.0.1``) — unwrapped before the IP-
      class check so an attacker can't bypass via IPv6 encoding.

    Hostname-based addresses are accepted without DNS resolution
    (consistent with our ``_validate_issuer_url`` policy — operator is
    trusted to configure a sensible IdP host).
    """
    parsed = urlparse(url)
