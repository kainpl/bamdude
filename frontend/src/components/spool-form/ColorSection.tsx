...
export function ColorSection({
  ...
  // preset selection matching and to derive the 6-char hex shown in the native
  // picker / draft input. #1545: alpha is tracked end-to-end so the ``Clear``
  // preset (00000000) stays distinct from black (000000FF).
  ...
  // Draft state for the manual hex input. Decoupled from ``formData.rgba``
  // so mid-typing values (1–7 chars) don't trigger the immediate auto-pad
  // that used to drop every keystroke past the first (upstream Bambuddy
  // #1407). The draft is committed to ``formData.rgba`` once it reaches 6 or
  // 8 chars, or on blur (where shorter drafts are padded with ``0`` so the
  // backend never sees a malformed rgba — preserves the #1055 invariant).
  // The ``useEffect`` re-syncs the draft whenever an external action (color
  // picker, swatch click, edit-mode load) changes the rgba.
  ...
  const selectColor = (
    hex: string,
    name: string,
    catalogExtras?: { extraColors?: string | null; effectType?: string | null },
  ) => {
    ...
    // #1340: catalog swatches are complete presets — picking one writes
    // BOTH the gradient and effect from the entry, so a solid catalog
    // preset clears any existing gradient/effect on the spool. Recent
    // colors and the hardcoded-fallback palette stay as plain hex
    // pickers (no ``catalogExtras`` → don't touch the fields), since
    // those swatches aren't full presets.
    if (catalogExtras) {
      updateField('extra_colors', catalogExtras.extraColors ?? '');
      updateField('effect_type', catalogExtras.effectType ?? '');
    }
    ...
    if (brand && !material) {
      ...
      if (byBrand.length > 0) {
        return byBrand.map(c => ({
          ...
          // #1340: propagate gradient + effect so the catalog click
          // applies the full preset look, not just hex + name.
          ...
          <div className="flex flex-wrap gap-1.5">
            {catalogSearchResults.map(color => (
              <button
                ...
                type="button"
                onClick={() => selectColor(color.hex, color.name, {
                  extraColors: color.extra_colors ?? null,
                  effectType: color.effect_type ?? null,
                })}
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                onChange={(e) => {
                  // Sanitize: drop ``#``, non-hex chars, uppercase. Accept up
                  // to 8 chars so power users can type an alpha byte (e.g.
                ...
                onBlur={() => {
                  ...
                  // (RGB + 1 alpha digit) pad the alpha nibble to 0 to reach a
                  // complete 8-char hex.
