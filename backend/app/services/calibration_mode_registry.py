...
MODE_STATE: dict[CaliMode, ModeState] = {
    ...
    # Phase 7 — PRODUCTION (2026-05-20). The builder loads the pass_n-
    # selected asset (flowrate-test-pass{1,2}.3mf), parses block names
    # from 3D/3dmodel.model, and bakes per-object print_flow_ratio
    # overrides parsed from each block's flowrate_<mod> name (BS-faithful,
    # byte-identical scaffolds). No post-slice patcher (BS has no
    # Calib_Flow_Rate engine case). The two-stage flow (coarse 9 / fine
    # 10) is fully wired end-to-end: m070 stores wizard args
    # (dispatch_args_json) on the parent session, and
    # _start_flow_rate_stage2 dispatches pass 2 automatically via
    # _run_calibration_print with pass_n=2 + baseline_flow_ratio set to
    # the operator's coarse pick. Verification signed off against an
    # Orca-desktop reference: every flow-rate-relevant CONFIG_BLOCK key
    # matched on both passes (filament_flow_ratio baseline, layer_height,
    # process patch, 12-key per-object override union).
