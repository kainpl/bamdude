...
async def backfill_spool_families(db: AsyncSession) -> dict[str, int]:
    summary = {"resolved_raw": 0, "resolved_legacy_column": 0, "unresolved": 0, "already_linked": 0}
    ...
    for spool in spools:
        ...
        if resolved.family:
            ...
            continue
        if spool.resolved_filament_id:
            # The FE-era column: trust it as origin=legacy rather than lose it.
            spool.filament_family_id = spool.resolved_filament_id
            summary["resolved_legacy_column"] += 1
            continue
