...
# --- Bambu Studio / OrcaSlicer layout (upstream 0eb083b3, #3135) -------------
#
...
# a part came back with N² copies of its triangles. Upstream that OOM-killed the
# server; here the preview child hits its RSS cap and the plate simply never
# gets a picture.
...
def _bambu_layout_3mf(
    ...
    part_file = (
        ...
        f"<resources>{objects}</resources><build/></model>"
    )
    wrappers = "".join(
...
class TestBambuLayoutGeometry:
    ...
    @pytest.mark.parametrize("part", ["3D/3dmodel.model", "3D/Objects/object_1.model"])
    def test_a_model_part_with_a_dtd_is_not_read(self, part):
        """The model is untrusted input, and ``resolve_entities=False`` does not
        cover attributes: libxml2 still expands an internal entity inside one
        (measured: ``x="&big;"`` came back as 1000). OPC forbids a DTD in package
        XML, so a part carrying one is refused outright — no thumbnail, the 3MF
        returned untouched."""
        from backend.app.services.plate_thumbnail import inject_plate_thumbnails_if_missing

        blob = _bambu_layout_3mf([("1", _grid(1)[0])])
        poisoned = io.BytesIO()
        with zipfile.ZipFile(io.BytesIO(blob)) as src, zipfile.ZipFile(poisoned, "w") as dst:
            for item in src.infolist():
                data = src.read(item.filename)
                if item.filename == part:
                    data = data.replace(
                        b'<?xml version="1.0" encoding="UTF-8"?>',
                        b'<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE model [<!ENTITY big "1000">]>',
                    )
                dst.writestr(item, data)
        poisoned_bytes = poisoned.getvalue()

        assert inject_plate_thumbnails_if_missing(poisoned_bytes) is poisoned_bytes
    ...
    def test_many_instances_are_decimated_to_the_face_budget(self, monkeypatch):
        ...
        blob = _bambu_layout_3mf([("1", t) for t in _grid(25)], mesh_xml=_sphere_mesh_xml())
...
def test_the_ceiling_is_the_preview_contracts_face_limit():
    """The plate is rendered in the preview child, whose face cap is the
    contract's ``FACE_LIMIT`` (spec: at most 200k faces reach the render, a
    failed simplification above it is a skip). The budget sits BELOW it: the
    per-mesh floor and a decimation that stops short both overshoot the budget,
    and with no slack every decimated plate would land just over and be skipped.
    """
    import backend.app.services.plate_thumbnail as pt
    from backend.app.services.preview_protocol import FACE_LIMIT

    assert pt._MAX_PLACED_FACES == FACE_LIMIT
    assert pt._RENDER_FACE_BUDGET < FACE_LIMIT
