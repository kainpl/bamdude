...
- **Waiting is an explicit choice, not an unexplained green button.** Print and Schedule check the complete filament assignment. If compatibility is unverified or refused, normal submission is disabled with an explanation; a separate confirmed action queues the job to wait, warning that it can hold up later jobs on that printer. Auto-Queue distinguishes an absent target model and offers deliberate storage for a future printer. A compatible but busy printer still accepts queue work. See the [routing guide](https://docs.bamdude.top/features/filament-routing/#preview).
...
- **Defects are recorded where you stand.** Every completed print card on an order page shows printed and defective and offers **Defects…**; on the printer card the finished print's counters sit beside **Clear plate** and **Repeat**. In Telegram the complete assessment stays a short-lived, cancellable draft, bound to its bot, chat and operator, until the whole plate is confirmed; typed counts must reply to their own prompt, and the form refuses to overwrite an archive changed while it was open. Clear/Repeat buttons carry both the named run and its gate generation, so an old card cannot clear a newer plate. Statistics gain **Defects by printer**. A print already counted into free stock has its shelf credit corrected when defects are recorded later, in either direction.
...
- **Isolated camera worker** — camera transport and FFmpeg run in one supervised local process; there is no in-process camera fallback. Worker failure leaves unrelated printing and queues available; jobs requiring a camera check may wait for recovery. System shows its state. This is isolation, not hardware acceleration or a guarantee against browser/network bottlenecks. See the [camera guide](https://docs.bamdude.top/features/camera/).
- **Isolated 3MF analysis** — filament-usage projections parse archive G-code in a supervised local service, share one cache per live print, and do not block the web server. The input file stays local; only a bounded result crosses the embedded broker. See [System diagnostics](https://docs.bamdude.top/features/system-info/).
...
- **ETA that counts the waiting** — an order's *Ready ≈* (shown only when every planned print can be dated) and a printer's *free at* include staggered starts (per group, with the slots heating right now), the plate-clear confirmation and the file transfer plus preheat, read from the very state the scheduler gates on rather than a second copy of it. A missing estimate or printer model names its reason instead of turning the routable subset into a completion date.
...
DEBUG=true uvicorn backend.app.main:app --reload --loop asyncio --timeout-graceful-shutdown 15
