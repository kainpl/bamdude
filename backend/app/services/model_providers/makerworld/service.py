...
Lets BamDude resolve a MakerWorld URL, enumerate plate/profile metadata, and
...
BamDude's existing Bambu Cloud bearer token (same SSO backend — no separate
...
class MakerWorldService(ProviderService):
    ...
    async def get_design(self, model_id: int) -> dict[str, Any]:
        ...
        BamDude: memoised for this service's life (one request) — the import
        asks for the design in ``get_download`` and again for its meta row.
        """
        model_id = int(model_id)
        cached = self._designs.get(model_id)
        if cached is None:
            cached = await self._get_json(f"/design/{model_id}")
            self._designs[model_id] = cached
        return cached
