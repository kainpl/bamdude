...
without opening a second MQTT session on the printer (reuses BamDude's
existing per-printer subscription — firmware inflight budget unaffected,
see upstream PR #1164).
...
Architecture (cached-as-base, not a separate fan-out stream):
  ...
  - **push_status** snapshots from the printer are CACHED here. The VP's
    ...
    ``sequence_id`` / ``gcode_state`` / etc. overridden). Single source of
    truth keeps BambuStudio's Send pre-flight happy.
...
def _resolve_target_to_ipv4(target: str) -> str | None:
    ...
    The printer client may be configured by IPv4 *or* by hostname/FQDN (e.g.
    ``p1s.fritz.box``) — the latter is common on home LANs with a DNS-providing
    router. The downstream ``net.info[].ip`` field is a 32-bit little-endian
    integer though, so a hostname can't round-trip through it; we have to pick
    *one* concrete IPv4 to write in.

    Returns None if ``target`` is empty, isn't parseable as IPv4, and DNS
    resolution fails — the caller logs that as the not-armed reason and re-tries
    on the next refresh tick (DHCP/DNS churn picks itself up) (#1429 follow-up).
    """
    if not target:
        return None
    try:
        ...
        pass
    try:
        # AF_INET filters to IPv4 only; the rewrite field is uint32 LE — there's
        # no IPv6 representation that fits.
...
class MQTTBridge:
    ...
    def __init__(
        ...
        *,
        ...
        # Last reason the net.info IP encoding failed to arm. Throttles the
        # "NOT armed" diagnostic to one line per state change — the refresh runs
        # every ~30s, so without this an idle-but-unarmed bridge would emit one
    ...
    async def _refresh_loop(self) -> None:
        ...
        ``BambuMQTTClient`` is destroyed and recreated on
        ``PrinterManager.connect_printer`` (e.g. printer config update). Without
        periodic refresh the bridge would lose fan-out after such a churn until
        the VP itself restarts.
        """
        try:
    ...
    def _refresh_ip_encoding(self, client) -> None:
        ...
        if not configured_target:
            ...
            _log_not_armed(
                f"target {configured_target!r} did not resolve to an IPv4 "
                "(hostname with no A record, or DNS returned only IPv6)"
            )
        else:
            ...
            # the net.info rewrite still arms. Without this, encoding never armed
            # on a default install and the slicer followed the real printer IP on
            # Send (#1429 residual, upstream cdc27eb5).
            if vp_ip in ("0.0.0.0", "", None):  # nosec B104
    ...
    def _resolve_client(self) -> None:
        ...
        # Trigger a fresh get_version + pushall against the printer so the
        # bridge cache populates immediately. BamDude itself queries these on
    ...
    def _on_printer_raw(self, topic: str, payload: bytes) -> None:
        ...
        (which trips BambuStudio's Send pre-flight consistency checks), we
        cache the latest real printer push_status here. The VP's existing 1 Hz
        synthetic push (which is what Send is built around) consults this
        cache and replaces its stub fields with real values when available.
        """
        if self._stopping:
            return
        ...
        # Race-free by construction: ``json.loads`` returns a fresh dict tree
        # per call so paho-thread mutations below cannot collide with prior
        # cached state held by the asyncio thread. ``_send_status_report``'s
        # shallow ``dict(cached)`` is also safe because nothing else writes to
        # the cached tree after assignment. The defensive deep-copy on store
        # below removes any future risk if a maintainer later re-enters the
        # cached dict to mutate it.
        ...
        if isinstance(print_data, dict) and print_data.get("command") == "push_status":
            ...
            # Note: ``ipcam.rtsp_url`` carries the real printer's IP. We pass
            # it through unchanged — the slicer uses it to fetch the live
            # camera stream directly from the printer. On the same LAN this
            # works as long as the slicer's stored access code matches the
            # printer's (i.e. configure the VP with the same access code as
        ...
        # the slicer (or BamDude) issued; the slicer matches by sequence_id and
