...
Auth shape (see :mod:`backend.app.services.orca_cloud` for the deep dive):
    ...
    POST /orca-cloud/logout
        Clear stored tokens (BamDude then has no token to use; the user can
...
# NOTE: unlike upstream, BamDude does not attach a router-level
# ``_cloud_api_key_gate`` dependency. Our cloud routes resolve the caller
# (JWT user or API-key owner) directly through ``RequirePermission`` on each
# handler — the same pattern our Bambu Cloud routes (routes/cloud.py) use — so
# the per-route ``RequirePermission(Permission.ORCA_CLOUD_AUTH)`` below is the
# single auth chokepoint. The route's own storage helpers already fall back to
# the Settings table when ``current_user`` is None (the ownerless-API-key
# path), mirroring ``cloud.get_stored_token``.
router = APIRouter(prefix="/orca-cloud", tags=["orca-cloud"])
...
async def _build_authenticated_service(
    ...
    # The service owns an httpx client from here on, and on success the caller
    # closes it — on failure nobody is ever handed it, so every raise below
    # used to leak one (upstream f3b1c591). BaseException: a cancelled request
    ...
    try:
        ...
        try:
            await svc.close()
        except Exception as close_error:  # noqa: BLE001 — never replace the error the caller needs
            logger.debug("Closing the Orca Cloud client after a failed build failed: %s", close_error)
        raise
    return svc
...
async def device_start(
    ...
    current_user: User | None = RequirePermission(Permission.ORCA_CLOUD_AUTH),
):
...
async def logout(
    ...
    BamDude will no longer have the token to use either way)."""
