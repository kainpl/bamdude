...
async def run_vp_diagnostic(vp: VirtualPrinter, instance) -> VPDiagnosticResult:
    ...
    if not running or not bind_ip:
        ...
        # The non-proxy bind server listens on BOTH 3000 (plain) and 3002 (TLS)
        # — slicers pick either path. Probing only 3002 missed half-dead VPs
        # where one listener failed to start and the other succeeded; report
        # port_bind as pass only when both probes succeed (upstream v0.2.4.5).
