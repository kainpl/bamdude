...
def is_expiry_401(response: httpx.Response) -> bool:
    ...
    a single stray rejection — including one from a background poll — so we trust
    only the documented expiry body. An unparseable / unsigned 401 is
    deliberately NOT expiry (upstream #2562 + its follow-up regression).
...
# challenges instead of the JSON the API normally produces (upstream Bambuddy
# #1575). The parse error that results is opaque — this helper detects the CF
# markers so we can surface an actionable message instead of the bare
# "Invalid response from Bambu Cloud".
_CF_INTERSTITIAL_USER_MESSAGE = (
    "Bambu Cloud is temporarily blocking automated requests from your network. "
    "This is a Cloudflare protection on Bambu Lab's side, not a BamDude issue. "
...
#
# The flag is keyed to the source IP and covers the region as a whole: the same
# 418 turns up on the login endpoint and on the design-service endpoints the
# MakerWorld import uses. It clears on its own after a few hours of quiet
# traffic, and there is no server-side solve — a CAPTCHA is designed to be
# unanswerable without a real browser, and the challenge id is of no use to us
# because we have nowhere to render the widget.
#
# ⚠️ It arrives as perfectly well-formed JSON, so ``_detect_cloudflare_challenge``
# above never fires on it, and the generic error path then lifted Bambu's own
# sentence out of ``error`` and showed it as a bare toast — "We need you to
# confirm you are not a robot", with no challenge, no explanation and nothing to
# click. Reported as a BamDude bug, understandably.
...
CAPTCHA_USER_MESSAGE = (
    ...
    "and there is no way to answer it from BamDude. Your email and password are not the "
...
# with a CAPTCHA challenge. Four attempts in eighteen seconds is exactly the
# traffic pattern that deepens the block: every extra request is more evidence
# for the thing that flagged us. Five minutes is short against the hours the
# block itself lasts — the point is not to wait it out here, only to stop us
# making it worse while the user reads the explanation.
_CAPTCHA_COOLOFF_SECONDS = 300.0

# API base URL -> monotonic time its cool-off expires. ⚠️ Keyed by base URL
# because the block lives at the edge in front of ONE region: being challenged
# on api.bambulab.com says nothing about api.bambulab.cn.
...
def is_captcha_challenge(response) -> bool:
    ...
    ⚠️ Requires the 418 status **and** a challenge marker in the body, so a bare
    418 is not reported to the user as "solve a CAPTCHA" — that would send them
    looking for a widget that was never there, which is the exact confusion this
    exists to end. A caller that wants to say something about an unmarked 418
    ...
    if isinstance(data, dict):
        # Field *names* count as well as their text: the challenge is identified
        # by carrying a ``captchaId`` at all, whatever that field says.
        parts = [str(key) for key in data]
        parts += [str(data[key]) for key in ("captchaId", "error", "message", "detail") if data.get(key)]
        haystack = " ".join(parts).lower()
    else:
        # Not JSON, or not an object — fall back to the raw body so a challenge
        # served as HTML is still recognised rather than reported as an
        # unexplained failure.
        try:
...
def captcha_cooloff_active(base_url: str) -> bool:
    ...
    challenge. Expired entries are dropped on the way past, so the map cannot
    grow past one entry per region."""
...
class BambuCloudService:
    ...
    def __init__(
        ...
        # with a stored token — the credential is dead and the caller wants to
        # record that. ``build_authenticated_cloud`` wires this to the persisted
        # flag, so every route that builds a service through it gets
        # invalidation for free rather than each one noticing 401s for itself.
    ...
    def is_authenticated(self) -> bool:
        ...
        Bambu's access token is opaque (no JWT claims to read an expiry out of),
        so the only authority on whether it still works is Bambu. This used to
        pretend otherwise: ``set_token`` stamped ``token_expiry = now + 30 days``
        every time a stored token was loaded, which re-derived the expiry from
        *now* on every request and made this property incapable of ever returning
        False. The UI reported "connected" indefinitely while every cloud call
        401'd (upstream #2562).

        ``token_expiry`` is now only set when we genuinely know it. Callers that
        need to know the token still *works* must ask Bambu — see
    ...
    async def _note_response(self, response: httpx.Response) -> bool:
        ...
        :func:`is_expiry_401`); a plain/transient 401 returns ``False`` and is
        left alone so it can't durably sign the user out. The durable flag is
        written at most once per service instance so a route making several calls
        doesn't write it repeatedly.
        """
    ...
    async def validate_token(self) -> bool | None:
        ...
        ``True`` accepted, ``False`` rejected (a signed expiry 401), ``None``
        unknown — Bambu was unreachable, answered 5xx, or returned a 4xx that
        isn't the expiry signature.
    ...
    def _captcha_refusal(self) -> dict:
        ...
        ⚠️ ``reason`` is what lets the UI tell this apart from a wrong password
        and render the explanation beside the access-token route, instead of
        flashing Bambu's own one-liner as a toast that then disappears.
        """
        return {
            "success": False,
    ...
    def _captcha_cooloff_holds(self, origin: str | None = None) -> bool:
        ...
        ⚠️ Keyed by the origin the call actually goes to. The TOTP step talks to
        ``bambulab.com`` while everything else talks to ``api.bambulab.com``, and
        a challenge seen on one must not strand somebody halfway through a
        ...
        logger.warning(
            ...
            "The challenge cannot be answered from BamDude and normally clears within a few hours.",
            origin,
        )
        return True
    ...
    async def login_request(self, email: str, password: str) -> dict:
        ...
        try:
            ...
            # No _note_response here: this is the LOGIN call. There is no stored
            # credential to invalidate yet, and a 401 means "wrong password",
            # which the branches below already surface.
            if response.status_code == 200:
    ...
    async def _fetch_csrf_token(self, web_origin: str) -> str | None:
        ...
        origin. A POST without the cookie is refused ``403 {"error": "CSRF
        error: missing_cookie"}`` **before the body is looked at**; with the
        ...
        Re-fetched per verification rather than cached: this client is
        process-wide and long-lived, so a stale cookie could disagree with the
        header we send — and the failure mode of that is the same misleading
        "Invalid code".
        """
        try:
            response = await self._client.get(
        ...
        # the cookie ride along on the POST below — the value is read here only
        # to echo it back in the header.
        try:
            token = self._client.cookies.get("bbl_csrf_token")
        except Exception:  # multiple cookies of that name across domains
            token = None
        if not token:
            logger.warning(
    ...
    async def verify_totp(self, tfa_key: str, code: str) -> dict:
        ...
        try:
            ...
            # ⚠️ Keyed on the web origin, not on ``self.base_url``. This step is
            # the only one that talks to bambulab.com, and a challenge seen on
            # api.bambulab.com must not strand somebody halfway through a
            # two-factor sign-in here.
            ...
            # which surfaced as a permanent, misleading "Invalid code" whatever
            # the user typed. api.bambulab.com — where every other call in this
            # service goes, including the email-code 2FA path — is not gated,
            # which is exactly why only authenticator sign-ins broke.
            ...
            # "Invalid code", which sends people chasing clock drift and
            ...
            if "csrf" in csrf_error.lower() or data.get("reason") in ("missing_cookie", "missing_header"):
                ...
                return {
                    "success": False,
                    "message": (
                        "Bambu Cloud refused the request before checking the code (security-token "
                        "mismatch). Try signing in again."
                    ),
                }
    ...
    def set_token(self, access_token: str):
        ...
        *now* on every request, for a token of entirely unknown age. That made
        ``is_authenticated`` a permanent True and is why the UI went on reporting
        "connected" long after Bambu had stopped accepting the token. A stored
        token's remaining life is unknowable from the token alone, so we record
        no expiry and let Bambu be the authority (upstream #2562).
        """
        self.access_token = access_token
        self.token_expiry = None
...
# via ``get_cloud_service()``. That pattern leaked region and token state
# across users (a China-region login would pin the singleton to
# api.bambulab.cn until the next explicit reset), so the singleton has been
# removed. Callers should construct a per-request
