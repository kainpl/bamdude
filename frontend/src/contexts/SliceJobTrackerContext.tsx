...
export function SliceJobTrackerProvider({ children }: { children: ReactNode }) {
  ...
  // Job ids whose terminal state has already been handled. `completeJob` shows
  // a toast and invalidates two query keys, so it has to run exactly once per
  // job however many callers reach it — see the poll loop below for how more
  // than one used to.
  ...
  const trackJob = useCallback(
    (id: number, kind: 'libraryFile' | 'archive', sourceName: string) => {
      ...
      // Re-tracking an id re-arms it. Ids come from a database sequence so this
      // cannot collide in practice; clearing here is what keeps the set from
      // becoming a permanent record of every job the session ever saw.
  ...
  const completeJob = useCallback(
    (job: TrackedJob, state: SliceJobState) => {
      // Guard, not an optimisation: everything below is a side effect the user
      // sees, and a second call would repeat all of it. Filtering activeJobs is
      // a state update, so `activeJobsRef.current` still names this job for
      // every poll round that resolved in the same batch.
  ...
  useEffect(() => {
    ...
    // not the previous one came back. Slicing a large project blocks the
    // backend for seconds at a time, and every tick that piled up during the
    // stall had already captured a snapshot naming the job as active. They all
    // resolved `completed` together and each called completeJob — which is how
    // one slice produced a stream of "Sliced X" toasts. Letting only one round
    // be in flight fixes that at the source, and stops queueing requests
    // against a backend that is already saturated.
    let polling = false;
