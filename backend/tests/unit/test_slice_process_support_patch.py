...
a per-quality one. BamDude passes the picked process preset via
...
class TestPatchProcessSupportSettings:
    ...
    def test_malformed_preset_json_returns_input_unchanged(self):
        # Symmetric to the bed-type patch's test suite. The slicer would
        # error on the preset anyway; the patch is a straight passthrough
        # so failure attributes to the original input.
        source = _make_3mf({"enable_support": "1"})
...
class TestDeclinedKeysAreNotReinstated:
    """What the user unticked stays unticked (upstream b1f5ec96, #2942).
    ...
    four support keys came out of the file whatever the ticks said. Our default
    still PRE-TICKS the designer's intent (the owner's call), so a file with
    supports on keeps switching supports on — until the user unticks them.
    """
...
class TestDeclinedSourceKeys:
    ...
    def test_no_list_at_all_declines_nothing(self):
        ...
        # never sends the field, Slicer Pipelines included -- cannot have
        # turned anything down, so the support carry-over stays as it was.
    ...
    def test_an_empty_list_declines_everything_on_offer(self):
        # Not the same answer as None: the list was shown, and nothing in it
        # was ticked.
