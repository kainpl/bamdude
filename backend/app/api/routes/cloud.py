...
from backend.app.core.auth import RequirePermission, _api_key_authority_or_none, is_api_key_token, security
...
# Credential storage is a service seam (upstream #2845); these names stay
# importable from here for the routes and tests that already use them.
...
async def clear_token(db: AsyncSession, user: User | None = None) -> None:
    ...
    When a user is provided, clears that user's credentials; otherwise
    (ownerless API-key fallback) the global Settings rows.
...
async def get_auth_status(
    ...
    existed - including tokens Bambu had been rejecting for months (upstream
    #2562). It now asks Bambu.
            ...
            try:
                ...
                for setting_id in unresolved_ids:
                    try:
                        ...
                        # A 400 here is the *expected* answer, not a fault, and the
                        # local-preset phase below exists to handle it. Two routine
                        # causes: many official presets are only addressable with a
                        # printer-variant suffix ("GFSA00" resolves, "GFSL05" does not
                        # — only "GFSL05_07" @BBL A1 does), and the bare ID is all the
                        # AMS reports; and personal "P…" presets belong to the Bambu
                        # account that sliced the file, so another account will never
                        # resolve them. One WARNING per AMS tray per tooltip refresh
                        # trains users to ignore the log. Anything else — expired
                        # token, 5xx, a connection failure — stays WARNING: a fault.
