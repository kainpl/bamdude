...
import { Plus, Edit2, Trash2, Globe, Check, X, RefreshCw, ExternalLink, Lock } from 'lucide-react';
        ...
        <Card key={provider.id}>
          <CardHeader>
            <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {provider.is_env_managed && (
                    <span
                      ...
                      title={t('settings.oidc.envManagedHint')}
                    >
                      <Lock className="w-3 h-3" /> {t('settings.oidc.envManaged')}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                {/* #2593: startup rewrites the env-managed row from BAMDUDE_OIDC_*
                    and the API answers 409, so offering these would promise a
                    change that cannot land. Hidden rather than disabled — a
                    greyed-out toggle invites a second attempt. */}
                {!provider.is_env_managed && (
                  <>
                    <Toggle
