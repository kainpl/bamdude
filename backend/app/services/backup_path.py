...
BamDude's systemd unit runs with ``ProtectSystem=strict`` (``deploy/bamdude.service``,
and the copy ``install/install.sh`` writes). That mounts the entire filesystem
read-only inside the service's own mount namespace and carves back out only
``ReadWritePaths=<install> <data> <logs>``. A backup output path on a NAS mount
is therefore read-only *to the service* while the operator's own shell writes to
it happily. The kernel reports this as ``EROFS``, not ``EACCES``, so the obvious
move — checking folder permissions — turns up nothing, and the real cause (our
own unit file) is the last place anyone looks (upstream #2544).
...
#   0::/system.slice/bamdude.service
#   0::/system.slice/system-bamdude.slice/bamdude@1.service
...
def systemd_unit_name() -> str | None:
    ...
    try:
        ...
        return "bamdude.service"
    match = _SERVICE_CGROUP.search(cgroup)
    return match.group(1) if match else "bamdude.service"
...
def classify_backup_dir_error(exc: OSError, backup_dir: Path) -> dict:
    ...
    if exc.errno == errno.EROFS:
        if unit:
            return {
                ...
                "message": (
                    f"{backup_dir} is read-only for the BamDude service. Its systemd unit runs with "
    ...
    if exc.errno in (errno.EACCES, errno.EPERM):
        return {
            ...
            "detail": detail,
            "remedy": None,
            "message": f"BamDude is not allowed to write to {backup_dir}. Check the directory's owner and mode.",
        }
    ...
    return {
        ...
        "detail": detail,
        "remedy": None,
        "message": f"BamDude cannot write to {backup_dir}: {exc}",
    }
...
def probe_backup_dir(backup_dir: Path) -> dict:
    ...
    Blocking on purpose — it does real filesystem I/O, on a path that may be a
    hung network mount. Callers on the event loop MUST run it through
    ``asyncio.to_thread``.
    """
    try:
        backup_dir.mkdir(parents=True, exist_ok=True)
        with tempfile.NamedTemporaryFile(dir=backup_dir, prefix=".bamdude-write-test-") as probe:
            probe.write(b"bamdude")
            probe.flush()
    except OSError as e:
