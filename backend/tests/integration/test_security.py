...
async def _setup_and_login(client: AsyncClient, username: str, password: str) -> str:
    """Create a new *admin* user (via the pre-seeded admin token) and log in as them.

    BamDude divergence from upstream: conftest already seeds a ``test_admin``
    user for every async_client, so the upstream pattern of POST /auth/setup
    returns 403 "already completed". Instead we use the pre-seeded admin's
    token (carried by ``async_client`` headers as a default ``Authorization``)
    to create the test user via POST /users/, attach them to the
    Administrators group (upstream tests expected the returned token to have
    full privileges, since setup always made an admin), then log in with
    an empty Authorization header to obtain a fresh JWT for that user.
    """
    ...
    # Upgrade the new user to admin by adding them to the Administrators group.
