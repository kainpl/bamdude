...
function buildLocalFilamentOptions(localPresets: LocalPreset[]): FilamentOption[] {
  ...
  const options: FilamentOption[] = filamentPresets.map(preset => {
    // Use the unique preset.id (stringified) as the code so each local
    // preset has its own identity. Earlier this was preset.filament_type
    // (e.g. "PLA") which collapsed every PLA local preset onto the same
    // code — picking any of them saved slicer_filament="PLA", a material
    // name the backend cannot resolve back to a specific preset row
    // (upstream Bambuddy #1387 / commit dd3e3f80). The backend handler
    // at inventory.py expects numeric IDs for local-preset slicer_filament
    // values. ``allCodes`` still carries the legacy filament_type so
    // ``findPresetOption`` resolves existing saved spools that have the
    // old material-name code.
...
// filaments — matching the behaviour of ConfigureAmsSlotModal. Earlier versions
// were precedence-based (cloud-only when cloud had any presets), which silently
// hid Local Profiles from users logged into Bambu Cloud — see upstream #1248.
//
// Cloud / local presets are *not* collapsed by base name: the spool form is
// printer-agnostic, so all P1S / X1C / A1 variants of "Bambu PLA Basic"
// render as separate rows. The user picks the one they want and its setting_id
// is what gets persisted. AMS Slot is per-printer (it filters); the spool form
// is union-of-all (it doesn't).
export function buildFilamentOptions(
  ...
  // 1. Cloud presets — each setting_id gets its own entry.
