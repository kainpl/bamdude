"""A print stage BamDude cannot name has to leave a trace at the default log level.
...
class TestQuietWhereItShouldBe:
    ...
    @pytest.mark.parametrize("sentinel", [-1, 255])
    def test_an_idle_sentinel_is_not_an_unnamed_stage(self, client, caplog, sentinel):
        """⚠️ Both are OUR OWN "not in a stage" markers, not firmware values —
        and -1 is the field's initial value. Every print would otherwise report
        one on the way out of its last real stage.

        ⚠️ 255 is our divergence from upstream, which excludes only -1: our
        macro tracking treats both as the idle marker (grep ``macro_executing``
        in the same transition block).
        """
        with caplog.at_level(logging.INFO):
            client._update_state({"stg_cur": 0})
            client._update_state({"stg_cur": sentinel})
        assert _stage_records(caplog) == []
