...
.DESCRIPTION
    - Uses default install directory C:\BamDude
    ...
    - Creates Start-BamDude.ps1
    - Optionally registers BamDude as Windows Service using NSSM
    - Optionally starts BamDude
...
function Show-BamDudeBanner {
    Write-Host ""
    Write-Host "============================================================" -ForegroundColor Green
    Write-Host "  ____                 ____            _        " -ForegroundColor Green
    Write-Host " | __ )  __ _ _ __ ___ |  _ \ _   _  __| | ___  " -ForegroundColor Green
    Write-Host " |  _ \ / _  | '_ ) _ \| | | | | | |/ _  |/ _ \ " -ForegroundColor Green
    Write-Host " | |_) | (_| | | | | | | |_| | |_| | (_| |  __/ " -ForegroundColor Green
    Write-Host " |____/ \__,_|_| |_| |_|____/ \__,_|\__,_|\___| " -ForegroundColor Green
    Write-Host "============================================================" -ForegroundColor Green
    Write-Host "            BamDude Setup - Install & Upgrade"               -ForegroundColor White
    Write-Host "============================================================" -ForegroundColor Green
    Write-Host ""
}
function Write-Log {
...
function Start-InstallerLogging {
    ...
    try {
        ...
        "============================================================" | Out-File -FilePath $script:LogFile -Append -Encoding UTF8
        "BamDude Installer started: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File -FilePath $script:LogFile -Append -Encoding UTF8
        "============================================================" | Out-File -FilePath $script:LogFile -Append -Encoding UTF8
...
function Read-Port {
    ...
    while ($true) {
        $inputPort = Read-Host "Enter BamDude port or press Enter for default [$DefaultPort]"
...
function Set-FolderPermissions {
    ...
    if (-not (Test-WriteAccess -Path $Path)) {
        throw "No write permission to '$Path'. Try another path, for example C:\Temp\BamDude, or check Windows Defender Controlled Folder Access."
    }
...
function Test-PythonVersion {
    ...
    try {
        ...
        if ($versionOutput -match "Python\s+(\d+)\.(\d+)\.(\d+)") {
            ...
            Write-Log "Found $versionOutput, but BamDude requires Python 3.10 or newer." "WARN" Yellow
        }
    }
    catch {}
...
function Move-LegacyRuntimeData {
    ...
    $legacyMappings = @(
        @{ Source = (Join-Path $BamDudeDir "bamdude.db"); Destination = (Join-Path $DataDir "bamdude.db") },
        @{ Source = (Join-Path $BamDudeDir "archive"); Destination = (Join-Path $DataDir "archive") },
        ...
        @{ Source = (Join-Path $BamDudeDir "icons"); Destination = (Join-Path $DataDir "icons") }
    )
    ...
    $legacyDataRoot = Join-Path $BamDudeDir "data"
    ...
    $legacyLogDir = Join-Path $BamDudeDir "logs"
...
function Register-BamDudeService {
    param (
        [Parameter(Mandatory = $true)]
        [string]$ServiceName,
    ...
    & $nssmExe set $ServiceName DisplayName "BamDude"
    & $nssmExe set $ServiceName Description "BamDude backend service"
    & $nssmExe set $ServiceName AppDirectory $BamDudeDir
...
try {
    ...
    if (-not $LocalOnly) {
        $lanQuestion = "Expose BamDude on the LAN? Choose No to bind only to this computer. Exposing BamDude on the LAN binds to all network interfaces. BamDude is unauthenticated by default. After installation, open Settings -> Security and enable auth before relying on LAN access."
    ...
    # ------------------------------------------------------------
    # Clone or update BamDude repository
    # ------------------------------------------------------------

    Write-Log "Preparing BamDude repository..." "INFO" Cyan

    $bamdudeRepoUrl = "https://github.com/kainpl/bamdude.git"
    $bamdudeFolderName = "bamdude"
    $bamdudeDir = Join-Path $installDir $bamdudeFolderName
    $dataDir = Join-Path $installDir "data"
    $appLogDir = Join-Path $installDir "logs"

    Write-Log "Repository target: $bamdudeDir" "INFO" Cyan

    New-Item -Path $dataDir -ItemType Directory -Force | Out-Null
    New-Item -Path $appLogDir -ItemType Directory -Force | Out-Null

    if (Test-Path $bamdudeDir) {
        $gitDir = Join-Path $bamdudeDir ".git"

        if (Test-Path $gitDir) {
            Write-Log "Existing BamDude Git repository found." "WARN" Yellow

            $updateExisting = Read-YesNo -Question "Do you want to update the existing repository with git pull?" -DefaultYes $true

            if ($updateExisting) {
                Push-Location $bamdudeDir
            ...
            Write-Log "Target directory exists but is not a valid Git repository: $bamdudeDir" "WARN" Yellow

            $removeBroken = Read-YesNo -Question "Remove this directory and clone again? This deletes '$bamdudeDir'." -DefaultYes $false

            if ($removeBroken) {
                Move-LegacyRuntimeData -BamDudeDir $bamdudeDir -DataDir $dataDir -LogDir $appLogDir
                Write-Log "Removing existing target directory..." "INFO" Cyan
                Remove-Item $bamdudeDir -Recurse -Force
            }
            else {
                throw "Cannot continue because '$bamdudeDir' already exists and is not a Git repository."
            }
        }
    }

    if (-not (Test-Path $bamdudeDir)) {
        ...
        Write-Log "Cloning BamDude repository..." "INFO" Cyan

        Push-Location $installDir

        & git clone --depth=1 --progress $bamdudeRepoUrl $bamdudeFolderName
        ...
        if ($gitCloneExitCode -ne 0) {
            throw "Failed to clone BamDude repository to '$bamdudeDir'."
        }
    }

    if (-not (Test-Path $bamdudeDir)) {
        throw "BamDude directory was not created: $bamdudeDir"
    }
    ...
    $venvDir = Join-Path $bamdudeDir "venv"
    ...
    $requirementsFile = Join-Path $bamdudeDir "requirements.txt"

    if (-not (Test-Path $requirementsFile)) {
        Pop-Location
        throw "requirements.txt was not found in $bamdudeDir"
    }
    ...
    if ($createFirewallRule) {
        ...
        if (-not $existingRule) {
            ...
            # Restrict to trusted profiles. BamDude ships with auth disabled by
    ...
    $startScriptPath = Join-Path $installDir "Start-BamDude.ps1"
    $runtimeLogPath = Join-Path $installDir "bamdude-runtime.log"
    $runtimeErrorLogPath = Join-Path $installDir "bamdude-runtime-error.log"

    $startScriptLines = @(
        '$ErrorActionPreference = "Stop"',
        '',
        "`$BamDudeDir = `"$bamdudeDir`"",
        ...
        '',
        'Set-Location "$BamDudeDir"',
        '',
        'Write-Output "Starting BamDude on port $Port"',
        'Write-Output "Bind address: $BindAddress"',
        'Write-Output "Working directory: $BamDudeDir"',
    ...
    $registerService = (-not $NoService) -and (Read-YesNo -Question "Register BamDude as a Windows Service?" -DefaultYes $true)

    if ($registerService) {
        Register-BamDudeService `
            -ServiceName "BamDude" `
    ...
    Write-Host "Repository path:   $bamdudeDir"
    ...
    Write-Host "Start-Service BamDude"
    Write-Host "Stop-Service BamDude"
    Write-Host "Restart-Service BamDude"
    Write-Host "Get-Service BamDude"
    Write-Host ""
    ...
    if ((-not $registerService) -and (-not $NoStart) -and (-not $script:Yes) -and (-not $script:Silent)) {
        $startNow = Read-YesNo -Question "Start BamDude now?" -DefaultYes $true

        if ($startNow) {
            Write-Log "Starting BamDude manually..." "INFO" Green
            Write-Host "Local URL:   http://localhost:$port"
            if ($bindAddress -eq "0.0.0.0") {
                Write-Host "Network URL: http://<this-computer-ip>:$port"
            }
            Write-Host "Press CTRL+C to stop BamDude."
            Write-Host ""
