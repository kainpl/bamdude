...
Found while investigating #2977 (upstream 4f10d155). BamDude sends a filament profile as a stub
...
def test_the_route_warns_only_on_the_profile_path():
    """The embedded-settings path sends no filament profiles for the bundle to
    resolve, so a vendor-less slot there is not this defect."""
    import inspect
    import re

    from backend.app.api.routes import library

    source = re.sub(r"\(\s+", "(", inspect.getsource(library))
    check = source.index("unresolved_filament_slots(result.content")
    guard = source.rindex("if not used_embedded_settings:", 0, check)
    assert source[guard:check].count("\n") <= 2
