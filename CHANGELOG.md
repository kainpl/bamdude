...
- **Background dispatch fully serialized.** Sending two prints to two different printers back-to-back could fail the second one mid-FTP with `sqlite3.OperationalError: database is locked` — both jobs raced on `INSERT INTO print_archives` and SQLite's single-writer semantics gave up after the busy-timeout. The dispatcher loop previously spawned parallel job tasks (one per printer); it now runs strictly one dispatch job at a time across the whole farm. Trade-off: the second printer waits a few extra seconds (until the first job's FTP upload + start command finish) before starting — preferable to a random "database is locked" failure mid-upload.
