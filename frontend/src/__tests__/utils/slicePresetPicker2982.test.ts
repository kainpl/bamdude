/**
 * Presets are matched on what they declare, not on what they are named
 * (upstream e9daa212, #2982).
 *
 * ⚠️ BamDude adaptation: "the same material" is the base type — PLA / PETG /
 * ABS as the slot and the preset state it — through the one equivalence group
 * dispatch routing uses (`filamentTypesCompatible`: PA-CF / PA12-CF / PAHT-CF).
 * Never a profile or a filament family: a Generic PETG and a Bambu PETG HF are
 * both PETG. An exact type still outranks an equivalent one.
 */
...
describe('the base-material group dispatch routing uses (BamDude)', () => {
  it('counts an equivalent type as the same material', () => {
    expect(statesDifferentMaterial({ filament_type: 'PA12-CF' }, 'PA-CF')).toBe(false);
    expect(statesDifferentMaterial({ filament_type: 'PAHT-CF' }, 'PA12-CF')).toBe(false);
  });

  it('still tells genuinely different materials apart', () => {
    expect(statesDifferentMaterial({ filament_type: 'PA-CF' }, 'PLA-CF')).toBe(true);
    expect(statesDifferentMaterial({ filament_type: 'PLA' }, 'PLA-CF')).toBe(true);
  });

  it('picks an equivalent type over a different material that fits the colour', () => {
    const presets = standard('filament', [
      { name: 'Bambu PETG HF @BBL A1', filament_type: 'PETG', filament_colour: '#000000' },
      { name: 'Bambu PA12-CF @BBL A1', filament_type: 'PA12-CF', filament_colour: '#FFFFFF' },
    ]);
    const pick = pickFilamentForSlot(presets, { type: 'PA-CF', color: '#000000' }, A1, index);
    expect(pick).toEqual({ source: 'standard', id: 'Bambu PA12-CF @BBL A1' });
  });

  it('lets the exact type outrank an equivalent one', () => {
    const presets = standard('filament', [
      { name: 'Bambu PA12-CF @BBL A1', filament_type: 'PA12-CF', filament_colour: '#000000' },
      { name: 'Generic PA-CF @BBL A1', filament_type: 'PA-CF', filament_colour: '#FFFFFF' },
    ]);
    const pick = pickFilamentForSlot(presets, { type: 'PA-CF', color: '#000000' }, A1, index);
    expect(pick).toEqual({ source: 'standard', id: 'Generic PA-CF @BBL A1' });
  });

  it('keeps any profile of the right base type eligible — never a family match', () => {
    // A Generic PETG and a Bambu PETG HF both state PETG: the partition is by
    // base type, so either can serve a PETG slot and colour decides.
    const presets = standard('filament', [
      { name: 'Generic PETG @BBL A1', filament_type: 'PETG', filament_colour: '#FFFFFF' },
      { name: 'Bambu PETG HF @BBL A1', filament_type: 'PETG', filament_colour: '#FF0000' },
    ]);
    const pick = pickFilamentForSlot(presets, { type: 'PETG', color: '#FF0000' }, A1, index);
    expect(pick).toEqual({ source: 'standard', id: 'Bambu PETG HF @BBL A1' });
  });
});
