...
#   docker pull ghcr.io/kainpl/bamdude:daily
#
# Prerequisites:
#   1. Log in to ghcr.io:
...
if [ "$SKIP_RELEASE" = true ]; then
    ...
    PULL_COMMANDS=""
    if [ "$PUSH_GHCR" = true ]; then
        PULL_COMMANDS="docker pull ghcr.io/kainpl/bamdude:daily"
    fi
    if [ "$PUSH_DOCKERHUB" = true ]; then
        if [ -n "$PULL_COMMANDS" ]; then
            PULL_COMMANDS="${PULL_COMMANDS}
# or
docker pull kainpl/bamdude:daily"
        else
            PULL_COMMANDS="docker pull kainpl/bamdude:daily"
        fi
    fi
