...
Translates between Spoolman's data model and BamDude's internal
...
async def create_spool(
    ...
    # (mirror update_spool). Spoolman has no ``color_name`` field on
    # Filament (#1357 — confirmed against FilamentUpdateParameters), so we
    # own the round-trip ourselves via ``spool.extra.bambu_color_name``.
...
async def update_spool(
    *,
    ...
    # A location_id edit also changes the denormalized storage_location string
    # Spoolman stores (upstream #1505). Resolve the final string here; the
    # ``clear_location`` kwarg below still fires when the resolution is empty.
    ...
    #
    # The old behaviour was always ``find_or_create_filament``, which
    # proliferated duplicate Spoolman filaments whenever the user changed
    # any field that made up the match key (material / subtype / brand /
    # color) — every edit minted a fresh row and orphaned the previous one
    # (upstream Bambuddy #1357 follow-up / commit 3b552094). To match
    # internal-mode behaviour (editing a spool does not proliferate new
    # entities), prefer PATCHing the current filament in place when it's
    ...
    if metadata_unchanged:
        ...
        if cur_filament_id and not shared:
            # Singleton filament — PATCH it in place so the user's edit
            # lands on the row their spool already points at instead of
            # orphaning it.
            patch_body: dict = {
                "name": desired_name,
                "material": material,
            ...
            # Filament is shared with other spools — PATCHing it in place
            # would silently rewrite their metadata too. Fall back to
            # find-or-create so only this spool's link moves.
            async with _translate_spoolman_errors():
    ...
    # set outside BamDude are preserved.
    tag_nulled = (
    ...
    # absent from the FilamentUpdateParameters schema in 0.23.1 (#1357),
    # so writing ``filament.color_name`` was a silent no-op that left
    # every edit looking "not saved". They all round-trip via extra and
    # get unpacked in ``_map_spoolman_spool``. Only writes when the
    # request explicitly set the field — passing null / omitting leaves
    # the existing extra entry untouched (write empty string to clear).
    ...
    if sf_set or sfn_set or cn_set:
        # Ensure extra fields are registered (Spoolman rejects PATCHes
        # with unknown keys with HTTP 400). Idempotent if startup already
        # ran this.
        if sf_set:
            await client.ensure_extra_field("bambu_slicer_filament")
        if sfn_set:
    ...
    try:
        ...
        if mqtt_client:
            ...
            if matching_fc:
                ...
                if fired:
                    logger.info(
                        "Spoolman assign: applied K-profile (k=%.3f, fc_id=%d) for spool %d on printer %d AMS%d-T%d",
                        matching_fc.pa_k_value or 0,
                        matching_fc.id,
...
def _k_profile_to_dict(p: SpoolmanKProfile) -> dict:
    """Manually map SpoolmanKProfile (link row + joined cache) → SpoolKProfileResponse-compatible dict."""
    fc = p.filament_calibration
    return {
        "id": p.id,
        "spool_id": p.spoolman_spool_id,
        "printer_id": p.printer_id,
        "extruder": p.extruder,
        "nozzle_diameter": str(fc.nozzle_diameter) if fc and fc.nozzle_diameter is not None else "0.4",
        "nozzle_type": fc.nozzle_volume_type if fc else None,
        "k_value": (fc.pa_k_value if fc and fc.pa_k_value is not None else (fc.flow_ratio if fc else None)),
        "name": fc.name if fc else None,
        "cali_idx": fc.cali_idx if fc else None,
        "setting_id": fc.filament_setting_id if fc else None,
...
async def save_spoolman_k_profiles(
    ...
    try:
        ...
        # Reuse the local inventory route's find-or-create helper — same
        # resolution chain (printer's live cali_idx → filament_calibration).
        from backend.app.api.routes.inventory import (
            _find_or_create_filament_calibration_for_link,
        )

        for profile in profiles:
            fc = await _find_or_create_filament_calibration_for_link(db, profile)
            if fc is None:
                continue
