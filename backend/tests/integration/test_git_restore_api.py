...
def _mock_private_repo_check():
    ...
    with patch(
        "backend.app.services.git_backup.git_backup_service.test_connection",
        new=AsyncMock(
            return_value={
                "success": True,
...
class TestCommitsEndpoint:
    ...
    async def test_404_when_not_configured(self, async_client: AsyncClient):
        response = await async_client.get("/api/v1/git-backup/commits")
    ...
    async def test_returns_commits_from_the_provider(self, async_client: AsyncClient):
        ...
        commits = [{"sha": "aaa1111", "message": "BamDude backup", "author": "BamDude", "date": "2026-07-02T10:00:00Z"}]
        with patch(
    ...
    async def test_limit_is_bounded(self, async_client: AsyncClient):
        await _create_config(async_client)
        assert (await async_client.get("/api/v1/git-backup/commits?limit=0")).status_code == 422
        assert (await async_client.get("/api/v1/git-backup/commits?limit=101")).status_code == 422
...
class TestPreviewEndpoint:
    ...
    @pytest.mark.integration
    async def test_404_when_not_configured(self, async_client: AsyncClient):
        response = await async_client.get("/api/v1/git-backup/restore/preview")
        assert response.status_code == 404
    ...
    async def test_reports_available_and_missing_categories(self, async_client: AsyncClient):
        ...
        with patch(
            "backend.app.services.git_restore.git_restore_service.preview",
            new=AsyncMock(return_value=preview),
        ):
            response = await async_client.get("/api/v1/git-backup/restore/preview?ref=aaa1111")
    ...
    async def test_rejects_refs_that_are_not_object_names(self, async_client: AsyncClient, ref):
        await _create_config(async_client)
        response = await async_client.get(f"/api/v1/git-backup/restore/preview?ref={ref}")
    ...
    async def test_defaults_to_head(self, async_client: AsyncClient):
        ...
        with patch("backend.app.services.git_restore.git_restore_service.preview", new=mock):
            response = await async_client.get("/api/v1/git-backup/restore/preview")
...
class TestRestoreEndpoint:
    ...
    @pytest.mark.integration
    async def test_404_when_not_configured(self, async_client: AsyncClient):
        response = await async_client.post("/api/v1/git-backup/restore", json={"categories": ["spools"]})
        assert response.status_code == 404
    ...
    async def test_applies_selected_categories(self, async_client: AsyncClient):
        ...
        with patch(
            "backend.app.services.git_restore.git_restore_service.run_restore",
            new=AsyncMock(return_value=outcome),
        ) as mock:
            response = await async_client.post(
                "/api/v1/git-backup/restore",
    ...
    async def test_rejects_empty_category_list(self, async_client: AsyncClient):
        await _create_config(async_client)
        response = await async_client.post("/api/v1/git-backup/restore", json={"categories": []})
        assert response.status_code == 422
    ...
    async def test_rejects_unknown_category(self, async_client: AsyncClient):
        await _create_config(async_client)
        response = await async_client.post("/api/v1/git-backup/restore", json={"categories": ["cloud_profiles"]})
        assert response.status_code == 422
    ...
    async def test_rejects_malformed_ref(self, async_client: AsyncClient):
        await _create_config(async_client)
        response = await async_client.post("/api/v1/git-backup/restore", json={"ref": "main", "categories": ["spools"]})
        assert response.status_code == 422
    ...
    async def test_defaults_overwrite_to_false(self, async_client: AsyncClient):
        ...
        with patch("backend.app.services.git_restore.git_restore_service.run_restore", new=mock):
            response = await async_client.post("/api/v1/git-backup/restore", json={"categories": ["spools"]})
...
class TestStatusExposesRestoreState:
    ...
    async def test_restore_running_is_reported(self, async_client: AsyncClient):
        ...
        git_restore_service._running_restore = True
        git_restore_service._progress = "Restoring spool inventory..."
        try:
            response = await async_client.get("/api/v1/git-backup/status")
        finally:
            git_restore_service._running_restore = False
            git_restore_service._progress = None
...
class TestRestoredArchivesAreVisibleToTheirOwner(TestOwnershipPermissionsSetup):
    ...
    async def test_the_owning_non_admin_can_open_a_restored_archive(
        ...
        from backend.app.services.git_restore import _CategoryTally, git_restore_service
        ...
        await git_restore_service._restore_archives(db_session, payload, False, _CategoryTally(), {})
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_carrying_the_owner_does_not_widen_who_may_read_it(self, db_session):
        """Control: the owner carry must not hand access to a non-owner.

        ⚠️ Asserted against ``_ensure_archive_visible`` rather than through a
        second operator's HTTP request, which is how upstream writes it. Their
        Operators do not hold ``archives:read_all``; **ours do, deliberately** —
        BamDude is a shared farm, and that is stated where the group is defined.
        So a second operator reading somebody else's archive is correct here and
        proves nothing. The guard is what actually decides, so the guard is what
        this pins: a caller with owner-only access still gets 404.
        """
        ...
        owner = User(username="restore_owner", password_hash="x", is_active=True)
        ...
        restored = (await db_session.execute(select(PrintArchive))).scalar_one()
        assert restored.created_by_id == owner.id, "the carry is what the rest of this rests on"

        stranger = User(id=owner.id + 1000, username="stranger", password_hash="x", is_active=True)

        # Owner-only access, someone else's row: 404, not 403 — 403 would leak
        # that the id exists.
        with pytest.raises(HTTPException) as caught:
            _ensure_archive_visible(restored, stranger, can_read_all=False)
        assert caught.value.status_code == 404

        # And the owner still gets it.
        assert _ensure_archive_visible(restored, owner, can_read_all=False) is restored
...
class TestRestoreDoesNotOpenTheMetricsEndpoint:
    ...
    async def test_restoring_prometheus_enabled_leaves_the_endpoint_shut(self, async_client: AsyncClient, db_session):
        ...
        await git_restore_service._restore_settings(
            db_session,
        ...
        assert "bamdude_build_info" not in response.text
    ...
    async def test_a_token_less_backup_leaves_the_endpoint_shut_too(
        ...
        tally = _CategoryTally()
        await git_restore_service._restore_settings(db_session, {"settings": payload}, overwrite=False, tally=tally)
        await db_session.commit()
    ...
    async def test_an_instance_with_its_own_token_still_gets_the_toggle_back(
        ...
        assert "bamdude_build_info" in authorised.text
...
class TestSettingsRestoreNeedsSettingsUpdate(TestOwnershipPermissionsSetup):
    ...
    ``git:restore`` alone let a role holding only Backup write, through a
    ...
    async def test_backup_only_role_cannot_restore_settings(self, async_client: AsyncClient, auth_setup):
        token = await self._token_for(
            async_client, auth_setup["admin_token"], "backuponly", ["git:backup", "git:restore"]
        )
    ...
    async def test_backup_only_role_cannot_restore_the_other_categories(
        ...
        token = await self._token_for(
            async_client, auth_setup["admin_token"], f"backuponly-{category}", ["git:backup", "git:restore"]
        )
        await _create_config(async_client, auth_setup["admin_token"])
    ...
    async def test_every_missing_permission_is_named_at_once(self, async_client: AsyncClient, auth_setup):
        ...
        """
        token = await self._token_for(
            async_client, auth_setup["admin_token"], "backuponly-all", ["git:backup", "git:restore"]
        )
        await _create_config(async_client, auth_setup["admin_token"])
    ...
    async def test_the_gate_is_per_category_not_a_blanket_demotion(self, async_client: AsyncClient, auth_setup):
        ...
        token = await self._token_for(
            ...
            "backupandinventory",
            ["git:backup", "git:restore", "inventory:read", "inventory:update"],
        )
        await _create_config(async_client, auth_setup["admin_token"])
    ...
    async def test_every_restorable_category_has_an_owning_permission(self):
        ...
        anywhere else — it simply restores under ``git:restore`` alone, which
        is the hole this whole class exists to close.
        """
        from backend.app.api.routes.git_backup import _CATEGORY_WRITE_PERMISSION
        from backend.app.schemas.git_backup import RestoreCategory
    ...
    async def test_a_role_holding_both_can_restore_settings(self, async_client: AsyncClient, auth_setup):
        ...
        token = await self._token_for(
            ...
            auth_setup["admin_token"],
            "backupandsettings",
            ["git:backup", "git:restore", "settings:read", "settings:update"],
        )
        await _create_config(async_client, auth_setup["admin_token"])
