...
All licenses are compatible with BamDude's GPL-3.0.

The `js/bamdude_adapter.js` bridge is a BamDude adaptation of the upstream
[Bambuddy](https://github.com/maziggy/bambuddy) `bambuddy_adapter.js`
(AGPL-3.0; same parent project as PrettyGCode). Diff scope vs upstream:
identifier rebrand only (`Bambuddy → BamDude`, `BambuddyPrettyGCode →
BamDudePrettyGCode`, `__bambuddy_*` path placeholders → `__bamdude_*`).
All API endpoints and wire formats match upstream so the vendored
PrettyGCode can speak to either fork's backend without touching the
third-party code.
