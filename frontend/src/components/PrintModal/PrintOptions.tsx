...
// Dual-nozzle-only calibration (H2D/H2D Pro/H2C/X2D) — appended only when the
// selected printer(s) are dual-nozzle (#1682). The MQTT layer forces "skip" on
// single-nozzle machines regardless, so hiding it avoids a misleading no-op.
const DUAL_NOZZLE_CALIBRATION_CONFIG = [
  { key: 'nozzle_offset_cali', labelKey: 'printModal.nozzleOffsetCali', descKey: 'printModal.nozzleOffsetCaliDesc' },
] as const;

// Plain boolean toggles.
const TOGGLE_OPTIONS_CONFIG = [
...
export function PrintOptionsPanel({
  ...
  const calibrationOptions = showDualNozzleOptions
    ? [...CALIBRATION_CONFIG, ...DUAL_NOZZLE_CALIBRATION_CONFIG]
    : CALIBRATION_CONFIG;
    ...
    <div className="mb-4">
      ...
      {isExpanded && (
        <div className="mt-2 bg-bambu-dark rounded-lg p-3 space-y-2">
          {calibrationOptions.map(({ key, labelKey, descKey }) => (
            <CalibrationModeControl
              key={key}
              label={t(labelKey)}
              description={t(descKey)}
              value={options[key]}
              onChange={(mode) => handleCalibrationMode(key, mode)}
          ...
          <div className="pt-2 mt-1 border-t border-bambu-dark-tertiary/60">
            ...
            {options.preheat_override !== 'off' && (
              <>
                ...
                {/* A typed 0 and a derived 0 do different things (upstream
                    #3041): the first asks for a bed-only preheat and still runs
                    the soak, the second skips the stage. Nothing here said so. */}
