...
Auth shape: OAuth 2.0 Device Authorization Grant (RFC 8628). BamDude is a
...
user approves a short ``user_code`` in their Orca Cloud settings; BamDude
...
``BamDude/<version>`` UA clears it. No TLS-fingerprint matching needed.
...
# Public client id for the device-pairing flow. Not a secret — it appears in
...
class OrcaCloudService:
    ...
    async def introspect(self) -> dict[str, Any]:
        ...
        after pairing to record the user's id for display in BamDude's UI."""
