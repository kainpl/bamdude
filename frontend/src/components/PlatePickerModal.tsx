      ...
      >
        <div className="flex-shrink-0 flex items-start justify-between gap-3 px-4 pt-4 pb-3 border-b border-bambu-dark-tertiary/40">
          <div className="min-w-0">
            <h3 className="text-white font-medium">{t('archives.platePicker.title', 'Pick a plate')}</h3>
            <p className="text-xs text-bambu-gray mt-1">
              {t('archives.platePicker.hint', 'Multi-plate file — choose which plate to operate on')}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-white font-medium truncate">
                    {plate.name
                      ? `${t('archives.platePicker.plateLabel', { index: plate.index, defaultValue: 'Plate {{index}}' })} — ${plate.name}`
                      : t('archives.platePicker.plateLabel', { index: plate.index, defaultValue: 'Plate {{index}}' })}
                  </p>
                  <p className="text-xs text-bambu-gray truncate">
                    {plate.objects.length > 0
                      ...
                      ? t('archives.platePicker.objectCount', {
                          count: plate.object_count,
