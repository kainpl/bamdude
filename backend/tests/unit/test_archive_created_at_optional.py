...
now ``datetime | None`` on the response models — a NULL row validates. (The
slim listing model was one of the three; it went with `/archives/slim` when
statistics moved to the server.)
...
from backend.app.schemas.archive import ArchiveDuplicate, ArchiveResponse
