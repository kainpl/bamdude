...
- **A queued file is independent of its original.** Accepting work makes one verified, immutable copy in `data/queue-sources/`. A laptop can sleep, an SMB share can disconnect, an external library file can be moved or deleted, and an archive can be cleaned without changing a job already in either queue. Identical bytes are stored once and remain until their last printer-queue or Auto-Queue job is removed; portable backups include the queue copies that their database snapshot names.
