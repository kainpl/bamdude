...
export function SecurityStatusCard() {
  ...
  const { data, isLoading, isError, refetch } = useQuery<EncryptionStatus>({
    ...
    // Bounded auto-recovery via refetchInterval backoff + manual recovery
    // via the "Retry" button rendered in the error branch below. Without
    // bounding, a single 5xx blip would kill the live status indicator
    // until a full page reload. The queryClient-level `retry` setting is
    // left untouched so operators (production) get the default 3 internal
    // retries while tests (which set retry:false) don't have to wait.
  ...
  if (isError || !data) {
    return (
      <Card id="card-mfa-encryption" data-testid="encryption-status-card">
        ...
        <CardContent>
          <div className="text-red-700 dark:text-red-400" data-testid="encryption-error">
            {t('settings.encryption.errorLoading')}
          </div>
          <button
            type="button"
            onClick={() => refetch()}
  ...
  if (data.decryption_broken) {
    ...
    statusLabel =
      data.key_source === 'env' ? t('settings.encryption.enabledFromEnv') : t('settings.encryption.enabledFromFile');
  ...
  // Show legacy-rows warning as a secondary alert when key is auto-generated
  // AND there are still unencrypted rows (both conditions can be true
  // simultaneously).
  ...
  return (
    <Card id="card-mfa-encryption" data-testid="encryption-status-card">
      ...
      <CardContent>
        <div className={`p-3 border rounded-lg ${severityClasses}`} data-testid="encryption-status">
