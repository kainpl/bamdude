...
class TestAssignSpoolTrayInfoIdx:
    ...
    async def test_gf_slicer_filament_kept(self, async_client: AsyncClient, printer_factory, spool_factory):
        ...
        # Slot must report a non-empty tray_type so the assign endpoint doesn't
        # defer MQTT (the empty-slot path is exercised separately).
...
class TestAssignSpoolEmptySlotPreConfig:
    """Pre-load assignment workflow: weigh-then-assign before the spool is in AMS.

    Bambu firmware silently drops ams_filament_setting / extrusion_cali_sel
    for unloaded slots — there's no filament context for the cali_idx to
    attach to. The endpoint persists the SpoolAssignment row with an empty
    fingerprint_type (the "pending config" marker) and skips the MQTT
    publish; on_ams_change re-fires the full configuration when filament
    is later inserted. Adapted from upstream Bambuddy ``b42aaca5`` (#1247).
    """
    ...
    async def test_empty_slot_skips_mqtt_but_persists_assignment(
        ...
        # Critical: no MQTT was published (firmware would silently drop it).
    ...
    async def test_on_ams_change_fires_config_when_pre_assigned_slot_loads(
        ...
        """Pre-config replay: SpoolAssignment with empty fingerprint + slot now loaded (state=11) → MQTT fires."""
        ...
        # Pre-existing assignment with empty fingerprint (the pre-load state)
