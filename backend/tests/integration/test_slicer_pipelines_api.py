"""Integration tests for the Slicer Pipelines CRUD API (#1425)."""
...
class TestSlicerPipelinesAPI:
    ...
    async def test_delete_is_soft(self, async_client: AsyncClient):
        """DELETE hides from list + GET-by-id but doesn't drop the row (run
        history must still resolve pipeline metadata)."""
        created = (await async_client.post("/api/v1/slicer-pipelines/", json=_payload())).json()
...
class TestSlicerPipelinesPermissions:
    """Permission gating — Viewers can read but not write (BamDude auth is always-on)."""

    @pytest.fixture
    async def viewer_token(self, async_client: AsyncClient) -> str:
        """A user in the Viewers group — holds pipelines:read only."""
        ...
        login = await async_client.post(
            "/api/v1/auth/login",
            json={"username": "pipe_viewer", "password": "ViewerPass123!"},
        )
        return login.json()["access_token"]

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_viewer_can_read(self, async_client: AsyncClient, viewer_token):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_viewer_cannot_create(self, async_client: AsyncClient, viewer_token):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_viewer_cannot_delete(self, async_client: AsyncClient, viewer_token):
        created = (await async_client.post("/api/v1/slicer-pipelines/", json=_payload())).json()
