...
- **Virtual Printer `auto_queue` mode** — new fourth VP mode alongside `print_queue` / `file_manager` / `proxy`. Slicer uploads land in the auto-queue with target model + required filaments auto-extracted from the 3MF; the scheduler then routes to whichever idle printer matches. `target_printer_id` is intentionally ignored in this mode (use `print_queue` for specific-printer routing). `auto_dispatch` toggle still controls whether the resulting auto-queue item is `manual_start` or runs on the next tick. Existing VP configs are unaffected — opt-in only.
