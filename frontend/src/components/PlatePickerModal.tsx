...
/**
 * Plate-picker modal shown by ArchivesPage's 3D-preview action when the
 * archive has more than one sliced plate (B.8 — port of upstream
 * Bambuddy `c44b6219` refactor of #963). Single-plate archives skip the
 * modal entirely and navigate straight into the viewer; source-only
 * archives (no sliced gcode) surface a noGcode toast instead — the
 * caller short-circuits both flows from `getArchivePlates(...)` before
 * mounting this component.
 *
 * Layout matches the Re-print modal's plate grid so users see the same
 * thumbnail/title/object-count pairing across reprints and previews.
 * Falls back to a Layers icon when a plate has no thumbnail (e.g. legacy
 * archives backfilled from disk before m023).
 */
