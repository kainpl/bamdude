...
Ports upstream Bambuddy's `test_ams_assignment_lock.py` (audit cycle
v0.2.3.2 → v0.2.4b1, item A.8).
...
SQLite's WAL serialises writes so the bug stayed latent there. It surfaced
when optional Postgres support landed and asyncpg started allowing true
...
class TestLockSerialisesConcurrentCallbacks:
    ...
    async def test_second_acquirer_waits_for_first(self):
        ...
        # mistakenly enter early; without the lock, "second-enter" would
        # land before "first-exit".
        for _ in range(5):
