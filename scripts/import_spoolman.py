#!/usr/bin/env python3
"""Import spools from Spoolman into BamDude inventory.

Usage:
    python scripts/import_spoolman.py --spoolman-url http://localhost:7912 --bamdude-url http://localhost:8000
    python scripts/import_spoolman.py --spoolman-url http://localhost:7912 --bamdude-url http://localhost:8000 --api-key YOUR_KEY
    python scripts/import_spoolman.py --spoolman-url http://localhost:7912 --bamdude-url http://localhost:8000 --dry-run
"""
...
def map_spool(sm_spool: dict) -> dict:
    """Map a Spoolman spool to a BamDude SpoolCreate payload."""
    filament = sm_spool.get("filament") or {}
    vendor = filament.get("vendor") or {}

    material = filament.get("material") or "PLA"
    color_hex = filament.get("color_hex") or ""
    # Spoolman color_hex is 6-char (#RRGGBB or RRGGBB), BamDude rgba is 8-char RRGGBBAA
    rgba = None
    if color_hex:
...
def create_bamdude_spool(bamdude_url: str, spool_data: dict, api_key: str | None = None) -> dict:
    """Create a spool in BamDude inventory."""
    url = f"{bamdude_url.rstrip('/')}/api/v1/inventory/spools"
    headers = {}
    if api_key:
...
def main():
    parser = argparse.ArgumentParser(description="Import spools from Spoolman into BamDude inventory")
    parser.add_argument("--spoolman-url", required=True, help="Spoolman URL (e.g. http://localhost:7912)")
    parser.add_argument("--bamdude-url", required=True, help="BamDude URL (e.g. http://localhost:8000)")
    parser.add_argument("--api-key", help="BamDude API key (required if auth is enabled)")
    ...
    for sm_spool in sm_spools:
        ...
        try:
            result = create_bamdude_spool(args.bamdude_url, payload, args.api_key)
            print(f"  Imported {label} -> BamDude spool #{result['id']}")
            created += 1
        except requests.RequestException as e:
