"""Unit tests for scheduled local backup service (upstream Bambuddy #884).

Adapted to BamDude rebrand:
- New filename prefix `bamdude-backup-` is the canonical one.
- Legacy upstream prefix `bambuddy-backup-` is still accepted by all read paths
  (list / resolve / delete / prune) so users who migrated from Bambuddy with
  pre-existing manual backups can still see and manage them.
"""
...
class TestPruneBackups:
    ...
    def test_prune_keeps_retention_count(self, tmp_path):
        service = LocalBackupService()
        for i in range(5):
            f = tmp_path / f"bamdude-backup-20260412-{i:06d}.zip"
            f.write_text(f"backup{i}")
        service._prune_backups(tmp_path, retention=3)
        remaining = list(tmp_path.glob("bamdude-backup-*.zip"))
    ...
    def test_prune_includes_legacy_prefix(self, tmp_path):
        """Pre-existing `bambuddy-backup-*` files count toward retention too."""
        service = LocalBackupService()
        # Mix new + legacy filenames; want to confirm both are pruned
        names = [
            "bamdude-backup-20260412-000001.zip",
            "bambuddy-backup-20260412-000002.zip",
            "bamdude-backup-20260412-000003.zip",
            "bambuddy-backup-20260412-000004.zip",
        ]
        for n in names:
            (tmp_path / n).write_text(n)
        service._prune_backups(tmp_path, retention=2)
...
class TestResolveBackupFile:
    ...
    def test_valid_filename(self, tmp_path):
        service = LocalBackupService()
        f = tmp_path / "bamdude-backup-20260412-120000.zip"
        f.write_text("data")
        result = service.resolve_backup_file(str(tmp_path), "bamdude-backup-20260412-120000.zip")
        assert result == f

    def test_legacy_prefix_accepted(self, tmp_path):
        service = LocalBackupService()
    ...
    def test_dotdot_blocked(self, tmp_path):
        service = LocalBackupService()
        result = service.resolve_backup_file(str(tmp_path), "..bamdude-backup.zip")
        assert result is None
...
class TestDeleteBackup:
    ...
    def test_delete_valid_backup(self, tmp_path):
        ...
        f = tmp_path / "bamdude-backup-20260412-120000.zip"
        f.write_text("data")
        result = service.delete_backup(str(tmp_path), "bamdude-backup-20260412-120000.zip")
        assert result["success"] is True
        assert not f.exists()

    def test_delete_legacy_backup(self, tmp_path):
        service = LocalBackupService()
        f = tmp_path / "bambuddy-backup-20260412-120000.zip"
        f.write_text("data")
...
class TestListBackups:
    ...
    def test_list_only_matching_files(self, tmp_path):
        service = LocalBackupService()
        (tmp_path / "bamdude-backup-20260412-120000.zip").write_text("a")
        (tmp_path / "bamdude-backup-20260412-130000.zip").write_text("bb")
        (tmp_path / "other-file.txt").write_text("ccc")
        result = service.list_backups(str(tmp_path))
        assert len(result) == 2
        assert all(
            r["filename"].startswith("bamdude-backup-") or r["filename"].startswith("bambuddy-backup-") for r in result
        )

    def test_list_includes_legacy_files(self, tmp_path):
        service = LocalBackupService()
        (tmp_path / "bamdude-backup-20260412-120000.zip").write_text("a")
        (tmp_path / "bambuddy-backup-20260412-130000.zip").write_text("b")
        result = service.list_backups(str(tmp_path))
        names = sorted(r["filename"] for r in result)
        assert names == ["bambuddy-backup-20260412-130000.zip", "bamdude-backup-20260412-120000.zip"]

    def test_list_sorted_newest_first(self, tmp_path):
        import time

        service = LocalBackupService()
        f1 = tmp_path / "bamdude-backup-20260412-120000.zip"
        f1.write_text("a")
        time.sleep(0.05)
        f2 = tmp_path / "bamdude-backup-20260412-130000.zip"
        f2.write_text("b")
        result = service.list_backups(str(tmp_path))
        assert result[0]["filename"] == "bamdude-backup-20260412-130000.zip"
        assert result[1]["filename"] == f1.name

    def test_list_includes_size(self, tmp_path):
        service = LocalBackupService()
        (tmp_path / "bamdude-backup-20260412-120000.zip").write_bytes(b"x" * 1024)
        result = service.list_backups(str(tmp_path))
