...
Also provides SSDP proxy functionality for proxy mode, where BamDude sits
...
class SSDPProxy:
    ...
    header changed to point to BamDude's IP on LAN B.

    This allows Bambu Studio on LAN B to discover the printer via BamDude.
    """
    ...
    def _rewrite_ssdp(self, data: bytes) -> bytes:
        ...
        - Location: changed to BamDude's remote interface IP
    ...
    async def _broadcast_to_remote(self) -> None:
        ...
        try:
            # Rewrite Location to point to BamDude's remote interface
            rewritten = self._rewrite_ssdp(self._last_printer_ssdp)
