"""Platform containment for the camera-worker harness child tree.
...
class CameraWorkerContainmentError(RuntimeError):
...
class WorkerContainment:
    ...
    def attach(cls, pid: int) -> WorkerContainment:
        ...
        except CameraWorkerContainmentError:
            raise
        except OSError as exc:
            raise CameraWorkerContainmentError("could not load Windows Job Object APIs") from exc
...
def _assign_windows_kill_on_close_job(pid: int) -> int:
    ...
    if os.name != "nt":  # pragma: no cover - protected by attach()
        raise CameraWorkerContainmentError("Windows Job Objects are unavailable on this platform")

    kernel32 = _windows_kernel32()
    job = kernel32.CreateJobObjectW(None, None)
    if not job:
        raise _windows_error("could not create camera-worker Job Object")
    process = None
    try:
        ...
        if not kernel32.SetInformationJobObject(
            ...
            raise _windows_error("could not configure camera-worker Job Object")

        process = kernel32.OpenProcess(_PROCESS_SET_QUOTA | _PROCESS_TERMINATE, False, pid)
        if not process:
            raise _windows_error("could not open camera-worker process for containment")
        if not kernel32.AssignProcessToJobObject(job, process):
            raise _windows_error("could not assign camera-worker process to Job Object")
        return int(job)
    except Exception:
...
def _windows_error(message: str) -> CameraWorkerContainmentError:
    return CameraWorkerContainmentError(f"{message} (Win32 error {ctypes.get_last_error()})")
