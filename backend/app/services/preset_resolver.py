"""Resolve a :class:`PresetRef` (source + id) to the JSON-string content the
slicer-api sidecar's ``/slice`` endpoint expects (Phase 1 of 0.5.x cycle).
...
:meth:`SlicerApiService.slice_with_profiles` accepts as
...
# BamDude then falls back to the embedded-settings path for every 3MF
...
async def resolve_preset_ref(
    ...
    Raises :class:`HTTPException` for any caller-facing error (invalid id,
    wrong preset type, cloud auth failure, network error fetching cloud detail).
    """
    if ref.source == "local":
...
async def _resolve_local(db: AsyncSession, ref: PresetRef, slot: str) -> str:
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Invalid local preset id for {slot}: {ref.id!r}",
        ) from None
...
async def _resolve_cloud(db: AsyncSession, user: User | None, ref: PresetRef, slot: str) -> str:
    """Fetch a single cloud preset detail. Permission gate matches the rest
    of the cloud surface (``CLOUD_AUTH``) so a user with ``LIBRARY_UPLOAD``
    ...
    # JSON lives under ``.setting``. The sidecar wants the preset content,
    # not the envelope.
    payload = detail.get("setting") if isinstance(detail, dict) else None
    if not isinstance(payload, dict):
        # Some endpoints return the preset at the top level instead of nested
        # under ``setting``. Fall back to the whole response in that case
        # rather than failing — the sidecar will reject it cleanly if the
        # shape is genuinely wrong, and we log the unusual response.
        logger.info(
...
def _resolve_standard(ref: PresetRef, slot: str) -> str:
    ...
    resolver walks ``BUNDLED_PROFILES_PATH/<category>/<name>.json`` and
    merges, yielding the full bundled preset without us round-tripping the
    content through BamDude."""
    ...
    return json.dumps(
        {
            # ``name`` must be set so the sidecar's compatibility checks see
            # a populated value. Reusing the bundled name keeps the resolved
