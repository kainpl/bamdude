...
describe('X1C model tests (single nozzle, real data)', () => {
  ...
  describe('computeAmsMapping', () => {
    ...
    // the per-nozzle hard filter must NOT apply. Upstream Bambuddy #1162.
    it('ignores nozzle_id when FTS is installed', () => {
      ...
      (status as unknown as { ams_extruder_map: Record<string, number> }).ams_extruder_map = {};
    ...
    it('matches external spools per-extruder on dual-nozzle without AMS', () => {
      ...
      (status as PrinterStatus).nozzles = [
        { nozzle_type: 'stainless_steel', nozzle_diameter: '0.4' } as PrinterStatus['nozzles'][number],
        { nozzle_type: 'stainless_steel', nozzle_diameter: '0.4' } as PrinterStatus['nozzles'][number],
      ];
    ...
    it('does not fabricate extruderId for single-nozzle with stub nozzles[1]', () => {
      ...
      (status as PrinterStatus).nozzles = [
        { nozzle_type: 'stainless_steel', nozzle_diameter: '0.4' } as PrinterStatus['nozzles'][number],
        { nozzle_type: '', nozzle_diameter: '' } as PrinterStatus['nozzles'][number],
      ];
      (status as PrinterStatus).ams_extruder_map = {};
    ...
    it('still applies nozzle filter when FTS object is null', () => {
      ...
      (status as unknown as { ams_extruder_map: Record<string, number> }).ams_extruder_map = { '0': 1, '1': 0 };
      (status as unknown as { fila_switch: unknown }).fila_switch = null;
