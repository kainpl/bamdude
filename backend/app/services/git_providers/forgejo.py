...
class ForgejoBackend(GiteaBackend):
    ...
    async def test_connection(self, repo_url: str, token: str, client: httpx.AsyncClient) -> dict:
        try:
            ...
            # Probe /user, but only a 401 here is conclusive: the instance
            # rejects the token outright, and saying so beats the 404 the repo
            # call may answer with instead (Forgejo v15+ hides private repos
            # behind 404 rather than 403).
            #
            # ⚠️ Every other status falls through to the repo check. A
            ...
            # That /user call is the only one in the whole provider layer.
            ...
            # Whether the token's identity was confirmed. Used only to word the
            ...
            # The repo call's own 401 is as conclusive as /user's, and a generic
            # "API error: 401" would send the operator looking at the repository
            # when the token is what is wrong.
