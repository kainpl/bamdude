...
# don't follow these prefixes; the psutil path filters on address class
...
def _get_network_interfaces_psutil() -> list[dict]:
    ...
    on Windows either, so both Linux code paths return empty here. psutil is
    already a dep (``psutil>=6.0.0``) and gives cross-platform name + IPv4 +
    netmask in one call.
    ...
    exclusion — users on Windows may legitimately bind a VP to a Hyper-V / WSL
