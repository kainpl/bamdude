...
Auth shape (see :mod:`backend.app.services.orca_cloud` for the deep dive):
    ...
    POST /orca-cloud/logout
        ...
        survives until its 1h expiry, but BamDude has no way to use it).
...
# handler — the same pattern our Bambu Cloud routes (routes/cloud.py) use —
# so the per-route ``RequirePermission(Permission.ORCA_CLOUD_AUTH)`` below is
# the single auth chokepoint. The route's own storage helpers already fall
# back to the Settings table when ``current_user`` is None (auth disabled),
# mirroring ``cloud.get_stored_token``.
...
async def auth_password(
    ...
    BamDude POSTs the credentials to Supabase and stores the returned
...
async def logout(
    ...
    either way, and BamDude will no longer have it to use)."""
