...
/**
 * Admin-only directory search + provision panel. Surfaced inside the create-
 * user modal as a "LDAP" tab so admins can onboard new users from the
 * directory without enabling auto-provision on login.
 *
 * Upstream Bambuddy #1298 / commit d6364646.
 */
...
export function LdapUserPicker({ onSuccess }: LdapUserPickerProps) {
  ...
  // request per character. 300ms matches debounces used elsewhere in the app.
  useEffect(() => {
  ...
  // Reset selection when the query changes so a stale selection from a
  // previous search can't be silently submitted.
  useEffect(() => {
    setSelectedDn(null);
