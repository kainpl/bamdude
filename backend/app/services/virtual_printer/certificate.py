...
class CertificateService:
    ...
    def _cert_matches_current_ca(self) -> bool:
        ...
        If the CA has been rotated since the per-VP cert was signed (e.g. the CA
        expired and was regenerated), the chain is broken even though both files
        exist on disk. ``ensure_certificates`` uses this to decide whether to
        regenerate.

        Uses real signature verification — our auto-generated CAs all share the
        same Subject DN ("Virtual Printer CA"), so a DN-only compare would
    ...
    def _get_or_create_ca(self) -> tuple[rsa.RSAPrivateKey, x509.Certificate]:
        ...
        # Save CA certificate and key. ``ca_key_path`` / ``ca_cert_path`` resolve
        # under ``shared_ca_dir`` (which may differ from ``cert_dir``), so the
        # parent to create is the CA file's parent — not ``cert_dir``. The old
        # ``cert_dir.mkdir`` created the per-VP subdir while the writes targeted
        # the CA dir; it only worked because the manager pre-creates both, so the
        # method itself was latent (upstream Bambuddy v0.2.4.5).
