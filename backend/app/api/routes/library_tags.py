...
Permission model (BamDude always-on auth):
...
NOTE: These user-authored tags are DISTINCT from ``LibraryFile.file_tags``
(the m036 computed system-badge column) and from ``PrintArchive.tags`` (the
archive CSV tag string). Do not conflate the three systems.
"""
...
# releases our requirements floor still allows, the app fails at IMPORT without
# this. Applies to every 204 route in a module with the future-annotations
# import — pinned by test_204_routes_declare_response_model.
