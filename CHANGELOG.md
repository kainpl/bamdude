...
- **P1P `0500_4003 "Failed to start print job"` after dispatch** — when the printer was slow to parse a `project_file` (P1P firmware 01.10.00.00 can take up to ~135 s on a freshly-uploaded 3MF), our verify-watchdog observed `gcode_state` still equal to the pre-dispatch value at timeout and force-reconnected the MQTT session as a half-broken-publish recovery. Reconnecting mid-parse caused the printer to abort with `0500_4003 "Failed to start print job"` (#1150). Both watchdogs (`PrintScheduler._watchdog_print_start` and `BackgroundDispatchService._verify_print_response`) now capture `gcode_file` from the printer's pre-dispatch status and, on timeout, only force a reconnect if `gcode_file` is *unchanged*. A change to a different `gcode_file` is the printer's "I accepted your file, just give me time to parse it" signal — leave the session alone. Unchanged `gcode_file` keeps the original half-broken-MQTT-session recovery (#887, #936). Caveat: re-dispatching the same gcode while the printer is still slow-parsing leaves `gcode_file` looking identical pre/post and the watchdog falls through to reconnect — accepted as the lesser evil to keep the half-broken-session recovery path working. Six new regression tests in `test_dispatch_gcode_file_discriminator.py` cover both watchdogs: skip-reconnect on `gcode_file` change, force-reconnect when unchanged, treat-as-landed when pre was None and current is non-None, fall-through reconnect when arg omitted (default-arg backward compat). Ported from upstream Bambuddy v0.2.4b1 (audit item A.3).
- **H2D Pro multi-plate dispatch double-/triple-fire** — scheduling 3 plates of a multi-plate file to the same H2D Pro caused the scheduler to fire all three `project_file` commands within ~60 s, even though the printer hadn't transitioned out of `FINISH` for the first one yet. The H2D Pro can sit at `FINISH` for 80–210 s after accepting `project_file` before `gcode_state` flips to `PREPARE`, and during that window the existing `PrinterQueue.status='printing'` DB seed in `print_scheduler.py::check_queue` was empirically missing the in-flight item — items 139/140/141 would all dispatch with status='printing' yet only the third actually triggered a state transition. User-visible: layer count flapping, all queued plates showing as printing simultaneously, MQTT disconnect storms, eventual print failure. Fix is a defensive in-memory dispatch hold layer in `print_scheduler.py`: when `_start_print` succeeds we record `(printer_id → (dispatched_at, pre_state, pre_subtask_id))`, and the next `check_queue` tick adds that printer to `busy_printers` until either (a) the watchdog observes a state/subtask transition (success path — release immediately past a 60 s minimum cooldown), or (b) a 180 s hard timeout expires (escape hatch for lost MQTT sessions). The minimum cooldown also prevents a spurious double-dispatch if the printer pulses through PREPARE→RUNNING→PREPARE in the first second after acceptance. Hold is purely additive — sits alongside the existing `PrinterQueue.status='printing'` seed query and `_is_printer_idle` checks, doesn't depend on DB row visibility, doesn't depend on `on_print_complete` firing correctly. Per-printer isolation: a hold on printer A never blocks printer B. The 90 s watchdog still owns the unhappy-path revert (queue item back to `pending` for retry) — this fix runs alongside it, not instead of it. 13 new unit tests in `test_scheduler_dispatch_hold.py` cover hold-record/release, state-transition release after cooldown, subtask-id advance release, status-unavailable keeps hold (printer disconnected post-dispatch — don't release on missing data), hard-timeout self-cleanup, transition-during-cooldown still holds (cooldown wins), no-pre-state fallback to time-based hold, and per-printer isolation. Ported from upstream Bambuddy v0.2.4b1 (audit item A.2).
- **Postgres restore aborted with `cannot drop table printers because other objects depend on it`** — `import_sqlite_to_postgres` (`backend/app/core/db_portable.py`, called by Settings → Backup → Restore on PG installs and by the SQLite→PG auto-migration path) called `metadata.drop_all`, which only enumerates ORM-defined tables and emits non-CASCADE `DROP TABLE`. When the live Postgres database carried orphan tables from removed features whose `*_printer_id_fkey` constraints still pointed at `printers`, Postgres refused to drop `printers`, the entire restore aborted before any rows landed, and the user was left without a working DB. The drop phase now executes `DROP TABLE IF EXISTS public.<name> CASCADE` on every table in the `public` schema (via a `pg_tables`-iterating PL/pgSQL `DO` block, after FKs have been stripped from the ORM metadata) before `metadata.create_all` rebuilds the schema. CASCADE is correct here — the user has explicitly chosen to wipe the DB and replace it from backup, so taking out orphan tables alongside ORM tables is intended behaviour. Restricted to `schemaname = 'public'` so a shared Postgres instance with non-BamDude data in other schemas is unaffected. SQLite restore path unchanged (file replacement via `shutil.copy2`). Three new regression tests in `test_postgres_restore_drop_cascade.py` mock the PG engine, run `import_sqlite_to_postgres` against a tiny SQLite source, and pin: (1) CASCADE-aware iteration over `pg_tables` is captured in the SQL stream — guards against silent regression to plain `metadata.drop_all`; (2) the iteration filters to `schemaname = 'public'`; (3) on SQLite the legacy `metadata.drop_all` path is taken with no CASCADE block. Ported from upstream Bambuddy v0.2.4b1 (audit item A.1).
