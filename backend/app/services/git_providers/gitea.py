...
class GiteaBackend(GitHubBackend):
    ...
    Plus the wrapped-commit-response shape from Gitea 1.24+: ``GET /git/commits/{sha}``
    returns the wrapped Commit schema (``commit.tree.sha``) where GitHub's
    same-named endpoint returns the unwrapped GitCommit (``tree`` at top
    level). ``_commit_tree_sha()`` accepts both shapes.
    """
