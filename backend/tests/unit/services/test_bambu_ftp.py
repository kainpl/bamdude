...
class TestAsyncWrappers:
    ...
    @pytest.mark.asyncio
    async def test_a_download_slower_than_the_timeout_still_completes(self, tmp_path, monkeypatch):
        """⚠️ The timeout bounds the wait for a worker SLOT, not the transfer.

        It used to bound both, which made ``ftp_timeout`` a ceiling on file
        size rather than a stall detector: measured against a real P1S, a
        21.6 MB 3MF takes 96 s at 231 KB/s and could never finish inside the
        30 s default, so the print was archived with "no 3MF available". A
        stall is still caught — by the socket timeout on each recv, which is
        what a stall actually looks like. BambuStudio splits it the same way
        (`Http.cpp`: CONNECTTIMEOUT 10, TIMEOUT 0 = never).
        """
        from backend.app.services import bambu_ftp

        bambu_ftp.BambuFTPClient._mode_cache.pop("127.0.0.1", None)
        local = tmp_path / "slow.bin"
        expected = b"arrived long after the old deadline"
        ...
        class FakeClient:
            ...
            def download_to_file(self, remote_path, local_path):
                time.sleep(1.0)  # 10x the caller's timeout
        ...
        result = await download_file_async(
            "127.0.0.1", "12345678", "/cache/slow.bin", local, timeout=0.1, printer_model="X1C"
        )
    ...
    @pytest.mark.asyncio
    async def test_a_worker_slot_that_never_comes_is_still_bounded(self, tmp_path, monkeypatch):
        """⚠️ What the cap is actually for (#2572).

        On a farm where offline printers keep every executor thread busy on
        dead connects, a caller holding a DB connection would wait for a slot
        forever and pin the pool. That wait stays bounded; only the transfer
        was freed.
        """
        from backend.app.services import bambu_ftp

        bambu_ftp.BambuFTPClient._mode_cache.pop("127.0.0.1", None)
        local = tmp_path / "never.bin"

        # Nothing ever signals "started", which is what a slot never granted
        # looks like from the caller's side.
        monkeypatch.setattr(bambu_ftp.threading.Event, "wait", lambda self, timeout=None: False, raising=False)

        result = await download_file_async(
            "127.0.0.1", "12345678", "/cache/never.bin", local, timeout=0.1, printer_model="X1C"
        )
