/**
 * Tests for the Restore from Git Backup modal.
 */
...
import { GitRestoreModal } from '../../components/GitRestoreModal';
  ...
  commits: [
    {
      ...
      message: 'BamDude backup - 2026-07-02 10:00:00 UTC',
      author: 'BamDude',
      date: '2026-07-02T10:00:00Z',
    },
    {
      sha: 'bbb2222ccc3333ddd4444eee5555ffff0aaa1111',
      message: 'BamDude backup - 2026-07-01 10:00:00 UTC',
      author: 'BamDude',
...
const mockPreview = {
  ...
  // English rendering as `detail` for i18next's defaultValue. Note the
...
describe('GitRestoreModal', () => {
  ...
  it('renders the title and commit picker', async () => {
    render(<GitRestoreModal onClose={vi.fn()} />);
  ...
  it('requires confirmation before sending the restore', async () => {
    let restoreCalls = 0;
    server.use(
      http.post('/api/v1/git-backup/restore', async () => {
        restoreCalls += 1;
  ...
  it('sends the selected categories and shows per-category results', async () => {
    ...
    server.use(
      http.post('/api/v1/git-backup/restore', async ({ request }) => {
  ...
  it('drops the selection while a newly-picked commit is still being inspected', async () => {
    ...
    let previewCalls = 0;
    server.use(
      http.get('/api/v1/git-backup/restore/preview', async () => {
        previewCalls += 1;
  ...
  it('reports a backend refusal such as the backup/restore mutex', async () => {
    server.use(
      http.post('/api/v1/git-backup/restore', () =>
        HttpResponse.json({
          success: false,
    ...
    expect(screen.queryByText(/Reload BamDude so the restored data appears/)).not.toBeInTheDocument();
  ...
  it('reports the categories a part-way failure already committed', async () => {
    server.use(http.post('/api/v1/git-backup/restore', () => HttpResponse.json(partialRestore)));
    render(<GitRestoreModal onClose={vi.fn()} />);
  ...
  it('explains a rejected commit-list request', async () => {
    server.use(
      http.get('/api/v1/git-backup/commits', () => HttpResponse.json({}, { status: 500 }))
    );
    render(<GitRestoreModal onClose={vi.fn()} />);
  ...
  it('closes via the close button', async () => {
    const onClose = vi.fn();
    render(<GitRestoreModal onClose={onClose} />);
