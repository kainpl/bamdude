...
async def create_spool_from_tray(db: AsyncSession, tray_data: dict) -> Spool:
    ...
    if tray_id_name and "-" in tray_id_name:
        ...
        if color_code and color_code[0] == "M":
            ...
            subtype = "Dual Color" if prefix == "A05" else "Gradient"
    ...
    # PLA Basic happens to come first in catalog insertion order. ``ORDER BY id``
    # makes the fallback path (empty ``tray_sub_brands`` — third-party spools /
    ...
    # resolve to "Black" (exactly the bug the cream rewrite in parse_ams_tray
    # used to paper over). Store "Clear" directly and let the frontend's
    # resolveSpoolColorName + hexToColorName render the swatch as a checkerboard.
...
async def find_matching_untagged_spool(db: AsyncSession, tray_data: dict) -> Spool | None:
    ...
    - **Brand**: only consider spools whose brand is unspecified or contains
      ...
      stored by the form's brand dropdown). Prevents a same-color
    ...
    - **Subtype**: prefer an exact match (e.g. AMS ``Basic`` → spool subtype
      ``Basic``), but fall back to a NULL-subtype spool — the form's Quick
      Add mode leaves subtype empty, so bulk-logged spools rely on this
      fallback to attract their RFID tag instead of duplicating on first
      AMS read.
