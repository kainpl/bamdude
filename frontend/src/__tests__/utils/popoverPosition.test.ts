...
describe('computePopoverPosition (#1669, iOS Safari visualViewport)', () => {
  ...
  it('flips above when visualViewport is shorter than innerHeight (iOS toolbar visible)', () => {
    ...
    // Trigger that fits under innerHeight but overflows under visualViewport:
    // bottom=400, height=320, total=724. 724 < 800-8 = 792 (no flip with
    // innerHeight), but 724 > 700-8 = 692 (flip with visualViewport).
    const trigger = { top: 380, bottom: 400, left: 400, right: 440 };
    const pos = computePopoverPosition({
      triggerRect: trigger,
      popoverWidth: 240,
