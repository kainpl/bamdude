/**
 * The "which hotend?" question a Filament Track Switch forces onto every load
 * (upstream 9500c046, BambuStudio's FeedDirectionDialog).
 */
...
describe('FeedDirectionModal', () => {
  ...
  it('will not confirm until a hotend is picked', async () => {
    ...
    // No default: a stray Enter must not feed an arbitrary hotend, which is why
    // BambuStudio starts with neither radio checked.
  ...
  it('disables a hotend already holding filament from this very slot', () => {
    render(
      <FeedDirectionModal
        {...defaultProps}
        extruderSlots={{ '0': { ams_id: 0, slot_id: 2, has_filament: true }, '1': empty }}
      />
    );
  ...
  it('offers a hotend pointed at this slot but holding nothing', () => {
    // Studio counts a hotend as holding a slot only when it has filament in it
    // (HasFilamentInExt) — an unloaded pointer is not a load.
    render(
      <FeedDirectionModal
        {...defaultProps}
        extruderSlots={{ '0': { ams_id: 0, slot_id: 2, has_filament: false }, '1': empty }}
      />
    );
  ...
  it('leaves both hotends offered when the loaded slot is a different one', () => {
    render(
      <FeedDirectionModal
        {...defaultProps}
        extruderSlots={{ '0': { ams_id: 1, slot_id: 2, has_filament: true }, '1': empty }}
      />
    );
