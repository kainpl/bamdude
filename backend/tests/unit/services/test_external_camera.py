...
class TestCaptureMjpegFrameWarmupSkip:
    """Regression for upstream Bambuddy #1177. Many MJPEG sources (notably
    go2rtc) emit a warm-up / black frame on the first byte that follows
    connection accept; `_capture_mjpeg_frame` must skip past it and return
    the second frame. Where the stream ends or times out before a second
    frame ever arrives the function falls back to the warm-up frame so
    callers still get *something* — returning None there would regress every
    code path that consumed the pre-fix behaviour (snapshot UX,
    plate-detection CV, finish photo, layer timelapse, Obico inference)."""
...
class TestSnapshotUrlOverride:
    """Upstream Bambuddy #1177 follow-up. When ``external_camera_snapshot_url``
    is set on the printer, every single-frame capture (notification thumbnail,
    finish photo, timelapse, plate-detect, Obico) must route through the plain
    HTTP-GET path on the snapshot URL instead of opening the live stream and
    skipping a warm-up frame. Sources that expose a dedicated frame endpoint
    (e.g. go2rtc's ``/api/frame.jpeg``) reliably return a clean image — the
    warm-up dance is only required for sources that don't, and bypassing it
    removes the inconsistency the reporter still saw after the warm-up fix
    landed."""
