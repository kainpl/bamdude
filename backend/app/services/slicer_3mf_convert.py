...
This module exposes helpers used by the cross-printer / cross-nozzle-class
re-slice path (#1325/#1493). The original two-pass cross-nozzle-class
config-splice idea was replaced by a simpler approach: forwarding the
sidecar's existing ``--arrange`` flag (see ``slicer_api.slice_with_profiles``
and ``_run_slicer_with_fallback`` in ``api/routes/library.py``). BambuStudio
itself reconciles the embedded ``project_settings.config`` against the target
printer when ``--arrange`` is on, so BamDude never reproduces that schema
logic locally.

What remains:
- ``count_plates_in_3mf`` — how many plates the source defines (slice-all loop).
- ``extract_source_printer_model`` — canonical model code of the source 3MF.
- ``merge_plate_3mfs`` — combine N single-plate slices into one multi-plate 3MF.
- ``substitute_unused_plate_filaments`` — keep BambuStudio's loaded-filament
  temperature validator happy on multi-plate projects.
"""
...
def count_plates_in_3mf(zip_bytes: bytes) -> int:
    """Number of plates the source 3MF defines, or ``0`` if it isn't a
    parseable 3MF / has no plate metadata. Used by the cross-class slice-all
    loop (#1493) to know how many ``--slice N`` calls to dispatch before
    merging the per-plate outputs into one multi-plate 3MF.
    """
    try:
        ...
        return 0
    # Each ``<plate>`` carries exactly one ``plater_id`` metadata entry.
    return len(re.findall(r'<metadata key="plater_id" value="(\d+)"', xml))


def extract_source_printer_model(zip_bytes: bytes) -> str | None:
    """Canonical short model code (e.g. ``"X1C"``, ``"H2D"``) for the 3MF's
    embedded ``printer_model`` field, or ``None`` if the input isn't a 3MF,
    has no embedded settings, the field is missing, or the model isn't
    recognised. Canonicalised via :func:`normalize_printer_model` (the raw
    field is ``"Bambu Lab H2D"``, not ``"H2D"``).
    """
...
def merge_plate_3mfs(
    ...
    Used by the cross-class slice-all loop (#1493) where each plate is sliced
    independently against the target printer (BS CLI's ``--arrange`` is
    project-wide, so a single ``--slice 0`` would consolidate every plate's
    objects onto one bed). Each input is a single-plate 3MF whose
    ``Metadata/plate_N.*`` entries already carry the right plate index.

    Strategy: the first plate's 3MF is the base (its project_settings.config,
    3dmodel, Auxiliaries carry forward); per-plate artifacts from the other
    inputs are overlaid; ``slice_info.config`` is re-assembled from each
    input's ``<plate>`` block. ``source_3mf_bytes`` provides a fallback for
    per-plate thumbnails the sliced outputs omit (BS with ``--arrange`` rarely
    writes a fresh per-plate preview).

    Single-element input is a passthrough. Empty input raises ``ValueError``.
    """
...
def substitute_unused_plate_filaments(source_3mf_bytes: bytes, plate_id: int | None, items: list[str]) -> list[str]:
    ...
    The modal still submits the unused rows' dropdown values, and BambuStudio
    validates every loaded filament for material compatibility — PLA in a used
    slot + ABS defaulted into an unused slot trips "the temperature difference
    of the filaments used is too large" (exit 194). Substituting unused entries
    with slot 1's filament keeps the array length intact (so the source 3MF's
    per-slot references stay valid) while making the loaded set homogeneous.

    No-op when: ``plate_id`` is None, the source isn't a valid 3MF, it carries
    no plate-extruder metadata, or ``items`` has fewer than 2 entries.
    """
    ...
    try:
        ...
        return items
    if not used:
        # No per-object extruder metadata → treat "no info" as "every slot
