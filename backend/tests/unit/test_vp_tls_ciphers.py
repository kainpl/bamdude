...
(plain RSA key exchange) on their TLS endpoints. On distributions whose
OpenSSL ``DEFAULT`` list has been narrowed by a system crypto policy
(Fedora / RHEL ``update-crypto-policies``, hardened Alpine builds), Python's
stock ``SSLContext`` ends up offering only ECDHE/DHE — no overlap with the
slicer's ClientHello, the handshake aborts, and the slicer reports a generic
``code=-1`` connect error.
...
def _cert_pair(tmp_path: Path) -> tuple[Path, Path]:
    ...
    CertificateService, so the SSL contexts under test are built exactly as
    they are in production (rather than mocking ``load_cert_chain``)."""
...
class _CaptureFakeServer:
    """Stand-in for the object ``asyncio.start_server`` returns, so start()
    can be driven far enough to build its SSLContext without binding a port."""
...
class TestTlsProxyServerCiphers:
    ...
    def test_create_server_ssl_context_offers_plain_rsa_aes_gcm(self, tmp_path):
        ctx = self._proxy(tmp_path)._create_server_ssl_context()
    ...
    def test_create_client_ssl_context_still_offers_plain_rsa_aes_gcm(self, tmp_path):
        ...
        ctx = self._proxy(tmp_path)._create_client_ssl_context()
