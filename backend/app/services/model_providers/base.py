...
Thingiverse, Printables, ...) whose files BamDude can resolve and import
...
from enum import StrEnum  # BamDude: Python 3.12 has it; there is no core.compat shim
...
class ModelProvider(ABC):
    ...
    #: The permissions this provider's read and import operations need.
    #: BamDude's routes gate on MakerWorld's own permission (every endpoint
    #: names one) and ``routes/makerworld._authorize_for_provider`` checks a
    #: provider's permission on top when it differs. Optional only so the base
    #: class has a default: a provider that leaves them unset is refused
