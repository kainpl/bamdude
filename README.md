...
- **A queued file is independent of its original.** Accepting work makes one verified, immutable copy in BamDude data storage. A laptop can sleep, an SMB share can disconnect, an external library file can be moved or deleted, and an archive can be cleaned without changing a job already in either queue. Identical bytes are stored once; portable backups include the queue copies that their database snapshot names.
