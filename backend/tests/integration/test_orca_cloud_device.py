...
token persistence, and status/logout â€” all in auth-disabled mode (the global
...
# BamDude divergence: upstream patches auth off for these calls, because its
# routes short-circuit when authentication is disabled. Auth is always on here
# (there is no ``core.auth.is_auth_enabled`` to patch), and the shared
# ``async_client`` fixture is already authenticated as an admin — so the calls
# below simply run as that user, which is the realistic path anyway.


@pytest.fixture(autouse=True)
def _dummy_shared_client():
    """Register a throwaway shared HTTP client so per-request
    OrcaCloudService() instances don't spin up (and leak) a real one â€” the
