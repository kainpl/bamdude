...
class TestExecuteHmsActionDispatch:
    ...
    def test_ignore_resume_sends_bambustudio_ignore_command_paused(self, client):
        ...
        # (DeviceManager.cpp:1450) — `command: "ignore"`, not `resume` and not
        # `idle_ignore`. The firmware handles both "skip this check on the next
        # attempt" and "resume the paused print" in one operation. The previous
        # code redirected to a plain resume, which caused wrong-plate to re-pause
        # 1-2 s after the user clicked Ignore (#1869). `err` is the DECIMAL int
        # representation of the hex error code.
    ...
    def test_ignore_resume_state_independent(self, client):
        # BambuStudio dispatches IGNORE_RESUME via `command_hms_ignore`
        # unconditionally — there's no PAUSE-vs-RUNNING branch. The previous code
        # special-cased PAUSE to a plain resume; the BambuStudio shape works in
        # both states.
    ...
    def test_ignore_no_reminder_uses_ignore_command_not_idle_ignore(self, client):
        # BambuStudio routes IGNORE_NO_REMINDER_NEXT_TIME and DONT_REMIND_NEXT_TIME
        # to the same `command_hms_ignore` as IGNORE_RESUME
        # (DeviceErrorDialog.cpp:596-602) — the "don't remind" half is the
        # firmware's responsibility, the wire shape is identical.
        client.state.state = "PAUSE"
    ...
    def test_no_reminder_next_time_uses_idle_ignore_type_zero(self, client):
        # NO_REMINDER_NEXT_TIME (distinct from IGNORE_NO_REMINDER_NEXT_TIME) is
        # BambuStudio's `command_hms_idle_ignore` with type=0
        # (DeviceErrorDialog.cpp:588-590). Dismisses the dialog without resuming.
        # The `err` is the same decimal-int format as the ignore command.
        client.state.state = "RUNNING"
