"""In-memory background dispatcher for slice jobs (Phase 1 of 0.5.x cycle).

Mirrors the shape of :mod:`background_dispatch` (the print-upload dispatcher)
but tailored for slicing: jobs are independent (no printer-busy gating),
short-lived (typically 5-60 s), and the result is a ``LibraryFile`` or
...
Multi-worker note: jobs live in a process-local dict, so a slice enqueued
on worker A can only be polled on worker A. That's fine for SQLite-backed
single-worker installs (the default); multi-worker deploys should pin
slice routes to one worker until we ship a DB-backed dispatcher.
"""
...
class SliceJob:
    ...
    # endpoint (security IDOR fix). None for API-key / auth-disabled callers
    # (no per-row identity); those jobs are visible only to READ_ALL pollers —
    # see slice_jobs.py::get_slice_job.
...
class SliceDispatchService:
    ...
    def set_progress(self, job_id: int, progress: dict[str, Any] | None) -> None:
        ...
        Called by the slice route's progress poller every ~1 s while the
    ...
    async def shutdown(self) -> None:
        """Cancel any in-flight job tasks at app shutdown.

        Pending tasks are cancelled and awaited so aiosqlite callbacks they
        scheduled don't fire against a closed loop in the lifespan teardown.
        Tasks cancelled before their coroutine ever started won't run their
        ``finally`` block, so we explicitly clear the registry afterward.
        """
