...
publishable key. BamDude generates the verifier/challenge/state, redirects the
...
callback URL back into BamDude (the loopback URL is the only ``redirect_to``
...
honest ``BamDude/<version>`` UA clears it. No TLS-fingerprint matching needed.
"""
...
# Honest client identity. Same posture as Bambu Cloud: identifies BamDude
...
_USER_AGENT = "BamDude/1.0 (+https://github.com/kainpl/bamdude)"
...
def generate_pkce() -> tuple[str, str, str]:
    ...
    handshake. The verifier is the secret kept by BamDude until the code
...
class OrcaCloudService:
    ...
    async def password_login(self, email: str, password: str) -> dict[str, Any]:
        ...
        allows). The actual server behaviour is what matters for BamDude
