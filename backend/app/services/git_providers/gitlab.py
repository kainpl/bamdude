...
class GitLabBackend(GitProviderBackend):
    ...
    async def test_connection(self, repo_url: str, token: str, client: httpx.AsyncClient) -> dict:
        try:
            ...
            # the purposes of the privacy safety check.
    ...
    def _encoded_project(self, repo_url: str) -> str:
        """The URL-encoded ``namespace/project`` path for ``/api/v4/projects/``.

        ⚠️ ``safe=""`` on purpose: a project inside a subgroup has slashes in its
        path, and GitLab wants them percent-encoded as part of ONE path segment.
        """
        owner, repo = self.parse_repo_url(repo_url)
