"""A slice with no printer start G-code must not become a print (upstream #2838).
...
The defect that produced such files was in the sidecar: BamDude sends a
bundled preset by name, and a resolver that walks only ``inherits`` never finds
the companion file holding the real start G-code, falling through to a
577-character generic stub. ⚠️ **Our own sidecar already resolves companions**
(``profile-resolver.ts``), so what this guards is an image built before that
fix — which is why the message says *rebuild*, not *update*. BamDude cannot see
that resolution happen either way, so this checks the one thing it can see: the
...
class TestTheMessage:
    def test_it_names_the_preset_and_the_actual_fix(self):
        ...
        # BamDude — saying so is the whole point of failing loudly.
        assert "sidecar" in message
        # ⚠️ Our divergence: we BUILD the sidecar from the fork's source, so
