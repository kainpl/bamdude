"""Regression tests for HomeAssistantService.list_entities domain filtering.

Ported from upstream Bambuddy commit 135b8fd9 / #1388.
...
Root cause: before the fix, the search path bypassed the domain filter
...
async def test_search_still_filters_to_allowed_domains():
    """Search must compose with the domain filter, not replace it (upstream #1388)."""
    entities = [
        {
