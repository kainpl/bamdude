...
describe('presetCompatibility', () => {
  ...
  it('resolves cloud / standard presets via the @BBL name suffix', () => {
    expect(
      presetCompatibility({ name: '0.20mm Standard @BBL X1C' }, 'process', X1C, index),
    ).toBe('match');
    expect(
      presetCompatibility({ name: '0.20mm Standard @BBL P2S' }, 'process', X1C, index),
    ).toBe('mismatch');
  });
...
describe('presetCompatibility — @BBL name fallback', () => {
  ...
  it('still resolves @BBL when the registry has not loaded yet (raw-token only)', () => {
    ...
    // work; codes that differ in form (X1C vs "X1 Carbon") gracefully fall
    // through to 'unknown'.
    expect(
      presetCompatibility(
