...
async def maybe_sync_spoolman_locations(db: AsyncSession, *, client=None) -> bool:
    ...
    if client is None:
        ...
        # BamDude's SpoolmanClient exposes both ``base_url`` (== url.rstrip("/"))
        # and ``api_url`` (== f"{base_url}/api/v1"). Compare on ``api_url`` per
        # the fork's client contract; derive the /api/v1 suffix so a cached
        # client for the same URL is reused instead of needlessly re-created.
        if not client or client.api_url != f"{cache_key}/api/v1":
