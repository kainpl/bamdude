...
- **Slicer-API Compose: explicit profiles for both services.** Both `orca-slicer-api` and `bambu-studio-api` now live behind explicit profiles. Pick `--profile orca` (OrcaSlicer only), `--profile bambu` (BambuStudio only), or `--profile all` (both). A bare `docker compose up -d` (no profile) now starts nothing — previously it started OrcaSlicer alone. **First-build workaround for Docker Desktop 4.71** (engine 29.4.1 / compose v5.1.x / buildx 0.33.x-desktop): the bundled `buildx bake` compose-bridge crashes immediately with `failed to execute bake: exit status 1` regardless of profile shape, and `COMPOSE_BAKE=false` doesn't help. Workaround documented in `slicer-api/README.md` and at the top of `slicer-api/docker-compose.yml` — set `DOCKER_BUILDKIT=0` + `COMPOSE_DOCKER_CLI_BUILD=0` for the build call (forces the legacy classic builder), then `up -d` works normally. Earlier Docker Desktop releases and Docker CE on Linux are unaffected.
...
- **Library file_type detection unified across all 7 write paths.** The upload route stored sliced `.gcode.3mf` files as `file_type='3mf'` while the slicer-output route stored `'gcode'` and the external-folder scan stored the unique `'gcode.3mf'` — three different values for the same physical container. Result was an inconsistent file-manager: an uploaded sliced 3MF showed the green "3MF" badge, the same file produced by the slicer showed the blue "GCODE" badge. New shared helper `services.library_helpers.detect_file_type` collapses every sliced 3MF (regardless of who wrote it) to `'gcode'`, matching the slicer's interpretation. Migration **m035** backfills existing rows. Also wired into the Virtual Printer's "save to library" path so files dropped via slicer's "Send to printer" land with the same primary type.
- **Composite `file_tags` array drives File Manager badges + filter.** New `library_files.file_tags` JSON column (migration **m036**) carries an ordered list of identity tags — `gcode` / `3mf` / `stl` / `step` (format), `multiplate` / `swap` (structure), `sliced` / `makerworld` / `project` (provenance). Sliced 3MFs carry both `gcode` AND `3mf` so the composite badge restores the visual distinction the m035 collapse removed. Computed at every write site by `compute_file_tags`; m036 backfill re-derives tags for existing rows. Frontend renders via a new `<FileTagBadges>` component (replaces three independent inline badge conditions in FileCard + FileListActions). New tag chip-row filter on the File Manager toolbar — additive on top of the existing primary-type dropdown, AND-filtered across selected chips, persisted in localStorage. Only chips for tags actually present in the loaded list render, so the row stays tight on installs that don't use every provenance source.
- **Slicer reachability indicators** on three surfaces, sharing one React Query cache + the new `GET /api/v1/slicer/health/{slicer}` endpoint (30 s in-process cache):
  - **Settings → Profiles → Slicer API** — small inline status next to each URL field (green check + version, or red X with error).
  - **Slice modal** — picker redesigned as two big card-buttons (matching the "Filament Tracking" pattern in Settings) with their own live health badges. Auto-locks to the only-healthy sidecar when one is down; user picks freely when both are reachable; offline cards are disabled. Submits the per-job `slicer` override whenever there's a concrete pick (user-chosen or auto-locked).
  - **System page** — new **Slicer Sidecars** section showing version + reachability + URL for each sidecar (auto-refresh 30 s alongside the rest of system info).
- **File-manager helpers consolidated onto `file_tags`.** The two filename-suffix helpers (`isSlicedFilename` / `isSliceableFilename`) duplicated across `FileManagerPage.tsx` (×2) and `ProjectDetailPage.tsx` are gone — replaced by `isSliced(file)` / `isSliceable(file)` / `isMultiPlate(file)` exported from `lib/fileTags`. Same source of truth as the badge row + chip-row filter, so the per-row Print button, the bulk-print "selected sliced files" picker, and the project-detail print-plan badge all agree on what counts as printable. Fewer string scans per render too — tag membership is O(1) on a short array, the suffix scan was O(n) on filename length and ran 6× per row in places.
...
- **MakerWorld import** — paste any MakerWorld model URL and BamDude resolves the design, lists its plates, and pulls the picked plate(s) straight into your library. New MakerWorld page in the sidebar; per-plate **Save** and **Save & Slice** actions; "Already in library" badges so you can see what's been imported. Re-imports of the same plate dedup to the existing library row. Anonymous browsing works without a Bambu Cloud token; downloads need one. New provenance badge in the file manager and archive views shows MakerWorld-sourced files with a clickable glyph that opens the original page (ratings, alternative plates, recommended filaments).
- **Server-side slicing via an OrcaSlicer / Bambu Studio sidecar** — Slice button on STL / 3MF / STEP / STP files in File Manager and Archives. Three preset tiers — cloud (per-user Bambu Cloud), local (your imported `.json` profiles), and standard (bundled) — merged with name-based dedup; per-slot multi-color filament selection; metadata-aware auto pre-pick. Pre-slice printer-mismatch warning prevents wrong-printer output when the source 3MF was sliced for a different machine. Live progress toast across the whole job. Optional `slicer-api/` Docker Compose stack ships in the repo so you can stand up the sidecar(s) with `docker compose up -d`. Disabled by default — enable in Settings → Profiles → Slicer API.
- **Library trash bin + archive trash bin** — deleting a library file or auto-purging an archive now sends it to a trash with a configurable retention window (default 30 / 365 days). Restore, hard-delete, and "Empty trash" actions in dedicated Trash pages (one for library, one for archives). Reference-aware hard-delete refuses to delete a library file still pinned by an active archive (returns a structured `library_file_pinned_by_archives` code). Admin **Purge old** modals with live preview of what will move to trash. Optional **Auto-purge** for both library and archives (off by default). External (read-only) library folders bypass the trash because the bytes live outside BamDude's control.
- **G-code injection per print job** — toggle in PrintModal that splices custom start/end G-code into the .3mf at the correct anchor (`; MACHINE_START_GCODE_END`), so the snippet runs *after* the printer has homed, heated, and meshed. Per-printer-model snippet library configurable in Settings → Printing → G-code Injection. Supports `{name}` placeholders against the .3mf header block, including a Prusa→Bambu alias map (`{max_layer_z}` → `{max_z_height}`, etc.) so PrusaSlicer-style snippets work unchanged. Failures are non-fatal: a typo or malformed 3MF logs and falls back to the unmodified upload. (#422)
- **Project external link + cover image** — Projects can now have a `url` and a cover image. Cover renders as a 3:1 hero strip on cards and detail page; an external-link icon next to the project name opens the URL in a new tab. URL is validated server-side (`http(s)://` only — `javascript:` / `data:` / `file:` and friends rejected). Cover image upload (jpg / png / gif / webp) with replace + remove buttons in the project edit form. (#1155)
- **Per-VP Tailscale Let's Encrypt cert + tailnet-FQDN SSDP advertise** — opt-in per virtual printer. When enabled and the local `tailscale` CLI is reachable, the VP provisions an LE cert via `tailscale cert` and broadcasts its tailnet FQDN over SSDP, so slicers connect to a hostname matching a trusted cert (no more click-through warnings or hand-installed CAs). Daily renewal loop covers near-expiry and SAN-mismatch from machine renames or tailnet migrations. (#1070)
- **Long-lived camera-stream tokens** for Home Assistant / Frigate / kiosks — per-user, named, revocable; hard 365-day max TTL. Plaintext is shown once at create time with copy-to-clipboard fallback for plain-HTTP LAN deployments. New "Camera API Tokens" panel in Settings → API Keys. (#1108)
- **Camera fan-out** — multiple browsers / tabs / kiosks watching the same printer's camera no longer kick each other off. A single upstream connection is shared across N subscribers; slow viewers drop frames instead of blocking the broadcaster. 5-second grace window on last-subscriber-leaves so a quick page refresh doesn't pay a fresh ffmpeg/RTSP handshake. (#1089)
- **Per-request trace IDs** — every HTTP request gets an 8-char hex correlation ID, threaded through to background tasks. Echoed back as `X-Trace-Id` so you can pin a server-side log search to the exact request a client made. Every log line gains a `[trace_id]` column.
- **Multi-colour gradients + visual-effect overlay for spools** — Spool form accepts up to 8 extra hex colour stops and a 10-option visual-effect dropdown (sparkle / silk / matte / glow / wood / marble / galaxy / rainbow / metal / translucent). Inventory swatches render as a 135° gradient when extra stops are set; effect is a rendering hint and doesn't affect MQTT or firmware. (#1154)
- **Per-spool category + per-spool low-stock override** — Spool form gets a free-text Category field with autocomplete from labels already in use, plus an optional Low-stock override (1–99%) so production and prototype spools can have different alert thresholds. Inventory page gets a Category filter chip when at least one spool is categorised. (#729)
- **Per-event ntfy priority** — AddNotificationModal exposes a per-event Priority dropdown (1..5) for `ntfy` providers. Unmapped events fall through to the server-side default. (#990)
- **Azure Entra ID OIDC support** — two new fields on OIDC providers: `email_claim` (default `email`, set to `preferred_username` or `upn` for Azure) and `require_email_verified` (default true). Auto-link safety guard refuses the unsafe combo (auto-link on + standard `email` claim + verified-check off) at schema, ORM, and Postgres-CHECK levels. (#1126)
- **Virtual Printer "Archive name source" toggle** — choose between metadata `print_name` (existing default) and bare filename for VP-archived prints. Useful for slicers that don't fill the title field meaningfully. (#1152)
- **"Not Printed" / "Printed" archive collections** — two new sidebar chips on the Archives page (server-side filter, pagination still works). (#1153)
- **"Awaiting printer…" toast for fast dispatch uploads** — when a multi-MB 3MF finishes uploading in under a second, the dispatch toast now shows "Awaiting printer…" with a pulsing bar instead of looking stuck at 100%.

### Fixed

- Windows `WinError 10054` traceback noise silenced when a peer (printer / MQTT broker / camera) RST'd a socket during cleanup. Real connection errors raised inside application tasks still surface normally. (#1113)
- Library uploads to writable external folders now actually write to the external mount instead of silently landing in internal storage under a UUID name. (#1112)
- Cross-boundary library file moves (managed↔external) now actually move the bytes, not just the DB row. Skipped-files response is now structured (`{file_id, code, reason}`) so the UI can explain why some moves didn't go through. (#1112 follow-up)
- AMS slot picker no longer hides Bambu Lab spools, and the assign affordance now appears on every vendor's slots. Empty slots no longer offer assign (the printer hasn't loaded a spool yet). (#1133)
- `index.html` is now served with `Cache-Control: no-cache, must-revalidate`, so deploys that rotate hashed asset filenames don't leave returning visitors loading stale documents. Hashed bundles under `/assets/*` keep their default long-lived caching.
- Uvicorn HTTP access log (POST/PUT/PATCH/DELETE only) is now piped into `bamdude.log` for incident triage.
- Spoolman iframe now shows a mixed-content warning + "Open in new tab" link when BamDude is on HTTPS but the Spoolman URL is HTTP, instead of silently blanking the iframe. (#1096)
- Defensive dedupe migration for the `settings` table on installs that came from very old Bambuddy imports — drops duplicate rows by `key` and adds the unique index that ORM `create_all` couldn't add to a pre-existing table.
- User-cancelled prints no longer surface as "Layer shift" failures. The HMS-derived `failure_reason` heuristic was matching on module alone (any 0x0C → "Layer shift", any 0x07 → "Filament runout", any 0x05 → "Clogged nozzle"); replaced with a curated 23-code short-code map. Cancel-echo HMS codes are also filtered from the printer-card "X problem" badge.
- Plate-clear flag flips now propagate to other tabs / scripts via the WebSocket broadcast — previously the flag updated in the DB but other UI subscribers stayed stale until the next coincidental status refresh. (#1128)
- RFID auto-match no longer creates duplicates of Quick-Add spools and no longer attracts non-Bambu spools — brand is now restricted to NULL or Bambu, and subtype falls back to NULL when no exact match exists. (#918)
- Reprint from Archive now back-fills `created_by_id` from the live print-session user when the source archive was auto-created from a printer-initiated print and had no attribution. The Statistics per-user filter no longer leaves these as unassigned. (#730 follow-up)
- Swagger UI / ReDoc no longer render blank under the strict global CSP. The `/docs`, `/redoc`, and `/docs/oauth2-redirect` paths now get a docs-scoped CSP that allows `cdn.jsdelivr.net`; every other route keeps the strict SPA policy.
- CameraPage now reads `?fps=N` from the URL (clamped 1–30, defaults to 15). (#1131)
- "Open in Slicer" now works for filenames with spaces on Windows / Linux — the file URL is `encodeURIComponent`-wrapped so the slicer's own `url_decode()` reaches the right path. (#1059)
- API Keys list — deleted keys now disappear synchronously instead of waiting for the next refetch.
- Inventory "Delete Tag" button renamed to "Clear RFID Tag" to match what it actually does (clears the RFID UUID off the spool record without deleting the spool).
- DB write lock no longer leaks on client disconnect mid-upload — `get_db` now catches `BaseException` and shields rollback + close, so `CancelledError` from a Starlette middleware tear-down still triggers cleanup. On Postgres the same code change averts a `QueuePool limit … overflow` symptom. Pool-cleanup noise from the cancellation path is silenced via a dedicated logging filter. (#1112 follow-up)
- Camera TLS proxy no longer logs a noisy traceback on uvloop when ffmpeg dropped its socket mid-flush — `RuntimeError` from uvloop's `UVStream.write` is now caught alongside the existing `ConnectionError` / `OSError` / `CancelledError`.
- H2C dual-nozzle detection now matches the post-2026 serial prefix `31B8B…` (was only matching the legacy `094…` prefix shared with H2D / H2C / H2S). (#1105)
- `/settings` and `/groups/{new,:id/edit}` no longer redirect delegated users to `/`. The hard-wired admin-only route guard was replaced with permission-based gates that match the sidebar visibility rules.
- Editing a custom group's permissions no longer appears to revert on reopen — the per-id detail cache is now primed with the PATCH response body. (#1083)
- Settings page no longer hits a 403 → toast → re-render → 403 loop for users with `settings:read` but not `settings:update`. Three call-site gates close the loop.
- Bare-metal `install.sh` now works on first-time Linux installs. The script clones as root then chowns to the service user, runs pip self-upgrade and `pip install -r requirements.txt` under the service user via the venv's pip path, and runs `npm ci && npm run build` under the service user via `sudo -H -u`.
- Spool auto-assign no longer raises `IntegrityError` on Postgres when two `ams_data` MQTT frames arrive ~30 ms apart — the auto-assign critical section is now wrapped in a per-printer asyncio lock. Unrelated printers stay fully parallel.
- Queue items no longer get stuck at `printing` when a print failed before reaching `RUNNING` (e.g. wrong nozzle size, AMS error, plate-clear gate). Completion detection now also fires on `FAILED` from `PREPARE` or `SLICING`, and the queue item's error message is populated from the live HMS error list (e.g. `"[0500_4038] The nozzle diameter in sliced file is not consistent with the current nozzle setting"` instead of a blank failure reason). (#1111)
- Background dispatch no longer reports "Print started successfully" when the printer never actually transitioned. The verify watchdog is now awaited inline; on timeout the dispatch job is marked failed with an actionable message ("Printer did not acknowledge print command — check for a pending HMS code, plate-clear prompt, or SD card fault"). The watchdog also accepts `subtask_id` advancing past its pre-dispatch value as a definitive "command landed" signal, and tolerates brief mid-window MQTT disconnects so transient telemetry gaps don't false-fail. (#1134, #1078)
- Reprint-from-archive no longer fails with `0500_4003 SD R/W` after a stuck dispatch. `force_reconnect_stale_session` now hard-resets the MQTT client (disconnect + loop_stop + fresh `connect`) so paho's QoS-1 publish queue can't replay a stale `project_file` on the new session. Routing is by caller thread — async callers get the hard reset, paho-network-thread callers keep the socket-close fallback to avoid a self-join deadlock. (#1136)
- P1P slow `project_file` parse no longer triggers a spurious mid-parse reconnect that aborted with `0500_4003`. Both watchdogs now skip the reconnect when the printer's `gcode_file` has changed (= "I accepted your file, just give me time to parse it"). (#1150)
- H2D Pro multi-plate dispatch no longer double-/triple-fires when the printer sits at `FINISH` for 80–210 s post-acceptance. A new in-memory dispatch hold layer keeps the printer in `busy_printers` until the watchdog observes a real state/subtask transition (or a 180 s hard timeout expires). Per-printer scope, so unrelated printers stay parallel.
- Postgres restore now drops orphan tables with CASCADE before re-creating the schema, so a backup restore on a database that picked up tables from removed features no longer aborts at "cannot drop table printers because other objects depend on it". SQLite restore path unchanged.

### Security

- `postcss` 8.5.6 → 8.5.12 (lockfile-only) — clears [GHSA-qx2v-qp2m-jg93](https://github.com/advisories/GHSA-qx2v-qp2m-jg93) (moderate-severity XSS in PostCSS's CSS Stringify output). PostCSS runs at build time only and BamDude doesn't pass user-controlled CSS through it at runtime, so the practical impact even pre-bump was nil — this is hygiene + clearing the `npm audit` warning.
