...
# BamDude: honest, versioned UA — see ``bambu_cloud._USER_AGENT`` and
# inv-honest-ua-cloud (Bambu Lab 2026-05-12 cloud-access policy: modifying
# AGPL code is allowed, impersonating official clients in cloud traffic is
# not). The Referer is kept because MakerWorld's CSRF / origin-check
# middleware uses it on some endpoints — a protocol requirement, not
# identity-faking.
...
def _s3_ssl_context() -> ssl.SSLContext:
    ...
    every other network call in BamDude — verifies against the bundled
