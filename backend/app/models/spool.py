...
class Spool(Base):
    ...
    # is ``max(0, weight_used - weight_used_baseline)``; the Inventory page's
    # "Reset usage to 0" action stamps ``baseline = weight_used`` so the
    # counter zeroes without disturbing remaining (= label_weight -
    # weight_used). Mirrors Spoolman's split between ``used_weight`` and
    # ``remaining_weight`` (upstream Bambuddy #1390, m075).
