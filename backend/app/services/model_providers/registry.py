...
The routing layer of the import API: a pasted URL goes through
:meth:`ModelProviderRegistry.find_for_url`, which asks each registered
provider ``supports_url`` and returns the one that owns it; an import names
its provider by ``source_type`` and goes through :meth:`get`. The endpoints
stay at ``/makerworld/*`` while MakerWorld is the only provider (BamDude).
"""
