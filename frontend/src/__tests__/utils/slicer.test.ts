...
describe('slicer utility', () => {
  ...
  describe('openInSlicer', () => {
    ...
    it('encodes filenames with spaces so the slicer receives %20 after url_decode (issue #1059)', () => {
      vi.spyOn(navigator, 'userAgent', 'get').mockReturnValue('Mozilla/5.0 (Windows NT 10.0)');
      // A download URL that already contains percent-encoded spaces — this is
      // how BamDude emits archive paths (the filename in the URL is URL-path-
      // encoded).
      ...
      // Each %20 in the input must become %2520 in the href so that after
      // the slicer's own url_decode() it comes back as %20 (preserving the
      // original URL). Without this, the slicer would decode %20 → literal
      // space and its subsequent HTTP fetch would fail.
