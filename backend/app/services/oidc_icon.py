"""OIDC provider icon fetcher (upstream Bambuddy #1333 / commit 8a7598f6).
...
- ``follow_redirects=False`` so the SSRF host allowlist (here:
  ...
  address.
- MIME whitelist (PNG / JPEG / WebP / GIF). SVG is rejected in v1 — XML
  payloads carry too many corner cases (xlink, external refs) for an MVP.
...
- 10 s timeout, matching the OIDC discovery / JWKS timeouts in
  ``routes/mfa.py``.
"""
...
class OIDCIconUnavailableError(OIDCIconError):
    ...
    Network timeouts, non-200 status, wrong content-type, oversized
    payload, redirects (we never follow), etc. Maps to a 400 at the API
    layer because the admin's input (the URL) is what's at fault.
    """
...
def _resolve_content_type(upstream_type: str, url_path: str) -> str:
    ...
    3. Distinct error when the header is missing entirely — empty quotes
       in a generic "unsupported content-type: ''" message was user-
       hostile in upstream's review.
...
async def fetch_icon(url: str) -> tuple[bytes, str, str]:
    ...
    exceeded — never buffers more than one chunk past the cap, so a
    hostile or misconfigured IdP serving a 500 MB payload cannot OOM the
    server.
    ...
    if parsed.scheme.lower() != "https":
        # Pydantic validator + assert_safe_public_https_url catch this
        # earlier, but the service is the last defence — refuse non-HTTPS
        # even if a future code path bypassed the validators.
    ...
    try:
        async with (
            ...
            if response.status_code != 200:
                # Any non-200 — including 301/302 redirects (we set
                # follow_redirects=False so the SSRF guard on the original
                # URL isn't bypassed by a redirect to a private address).
                raise OIDCIconUnavailableError(
        ...
        # ``httpx.InvalidURL`` is a sibling of ``httpx.HTTPError`` (no
        # HTTPError in between in the MRO). Fires at send-time for URLs
        # that ``urlparse`` accepts but httpx refuses — typically null
        # bytes or control chars. Map to URL-error path so the admin
