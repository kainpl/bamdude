...
// user input (especially the modal's close button), and avoid the "page
// unresponsive" dialog while we crunch through large 3MFs in straight-line
// JS. setTimeout(_, 0) is enough — we just need to surrender control so
// queued tasks run (#1412).
function nextTick(): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, 0));
}

// Yield once per N iterations of a hot loop — ~5-10 ms of work per batch on a
// typical desktop: fine-grained enough to keep frames flowing, coarse enough
// not to drown the loop in setTimeout dispatch overhead.
...
async function parse3MF(arrayBuffer: ArrayBuffer): Promise<Parsed3MFData> {
  ...
  for (let i = 0; i < objectElements.length; i++) {
    ...
    // throughout the parse (#1412). Inner loops yield on their own.
    if (i > 0) {
      await nextTick();
    }
    const objEl = objectElements[i];
    ...
    for (let j = 0; j < componentElements.length; j++) {
      // Yield before each component — each triggers another async file load +
      // DOM parse + vertex/triangle iteration. Multi-colour "parted" statues
      // from MakerWorld can have dozens; without this the chain runs as one
      // long synchronous burst and freezes the modal close button (#1412).
      await nextTick();
