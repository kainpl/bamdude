 ...
 * BamDude isn't on the user's localhost), and the user copies the URL from
...
export function OrcaCloudView() {
  ...
  // Configured BamDude printers — fed into the profile-view's printer
  ...
  const startAuthMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setPasteError(null);
      setMode('paste');
      // Open in a new tab so the user can keep BamDude open in their
