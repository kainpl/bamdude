...
- **Defects are recorded where you stand.** Every completed print card on an order page shows printed and defective and offers **Defects…**; on the printer card the finished print's counters sit beside **Clear plate** and **Repeat**. In Telegram the complete assessment stays a short-lived draft, bound to its bot, chat and operator, until the whole plate is confirmed; typed counts must reply to their own prompt, and clear/repeat buttons can affect only that named run — an old card cannot clear a newer plate. Statistics gain **Defects by printer**. A print already counted into free stock has its shelf credit corrected when defects are recorded later, in either direction.
