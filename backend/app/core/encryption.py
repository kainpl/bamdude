...
def _load_or_generate_key() -> tuple[str | None, _InternalSource]:
    ...
    # Use os.open(O_WRONLY|O_CREAT|O_EXCL, 0o600) to avoid the TOCTOU window
    # between write_text() (umask-respecting) and chmod() — the key is
    # created with 0o600 from the start, never world-readable.
    ...
    try:
        ...
        try:
            ...
            os.close(fd)
        # Some filesystems (Windows, SMB, FUSE without uid mapping) silently
...
def _get_fernet():
    ...
    # Collapse internal failure causes to public "none" while keeping the
    # differentiated source for the warning path below.
    _key_source = "none" if internal_source.startswith("none") else internal_source

    if key is None:
        if not _warn_shown:
            # Only emit the "DATA_DIR not writable" warning when that's
...
def mfa_decrypt(value: str) -> str:
    ...
    if not value.startswith(_FERNET_PREFIX):
        # Warn when a key IS configured but the stored value is plaintext.
