"""API routes for Git backup (GitHub and GitLab)."""
...
@router.get("/config", response_model=GitBackupConfigResponse | None)
async def get_config(
    db: AsyncSession = Depends(get_db),
    _: User | None = RequirePermissionIfAuthEnabled(Permission.GIT_BACKUP),
):
    """Get the current Git backup configuration."""
...
@router.post("/config", response_model=GitBackupConfigResponse)
async def save_config(
    config_data: GitBackupConfigCreate,
    db: AsyncSession = Depends(get_db),
    _: User | None = RequirePermissionIfAuthEnabled(Permission.GIT_BACKUP),
):
    ...
    if config:
        ...
        if config.schedule_enabled:
            config.next_scheduled_run = git_backup_service._calculate_next_run(config.schedule_type)
        else:
            config.next_scheduled_run = None

        logger.info("Updated Git backup config: %s", config.repository_url)
    else:
        # Create new
        ...
        db.add(config)
        logger.info("Created Git backup config: %s", config.repository_url)
...
@router.patch("/config", response_model=GitBackupConfigResponse)
async def update_config(
    update_data: GitBackupConfigUpdate,
    db: AsyncSession = Depends(get_db),
    _: User | None = RequirePermissionIfAuthEnabled(Permission.GIT_BACKUP),
):
    """Partially update Git backup configuration."""
    result = await db.execute(select(GitBackupConfig).limit(1))
    config = result.scalar_one_or_none()
...
async def delete_config(
    ...
    """Delete the Git backup configuration and all logs."""
    result = await db.execute(select(GitBackupConfig).limit(1))
    config = result.scalar_one_or_none()
    ...
    logger.info("Deleted Git backup config")

    return {"message": "Configuration deleted"}


@router.post("/test", response_model=GitTestConnectionResponse)
async def test_connection(
    ...
    result = await git_backup_service.test_connection(repo_url, token, provider=provider, api_base_url=api_base_url)
    return GitTestConnectionResponse(**result)


@router.post("/test-stored", response_model=GitTestConnectionResponse)
async def test_stored_connection(
    db: AsyncSession = Depends(get_db),
    _: User | None = RequirePermissionIfAuthEnabled(Permission.GIT_BACKUP),
):
    """Test Git provider connection using stored configuration."""
    result = await db.execute(select(GitBackupConfig).limit(1))
    config = result.scalar_one_or_none()
    ...
    test_result = await git_backup_service.test_connection(
        config.repository_url, config.access_token, provider=config.provider, api_base_url=config.api_base_url
    )
    return GitTestConnectionResponse(**test_result)


@router.post("/run", response_model=GitBackupTriggerResponse)
async def trigger_backup(
    ...
    backup_result = await git_backup_service.run_backup(config.id, trigger="manual")

    return GitBackupTriggerResponse(**backup_result)


@router.get("/status", response_model=GitBackupStatus)
...
@router.get("/logs", response_model=list[GitBackupLogResponse])
async def get_logs(
    ...
    logs_result = await db.execute(
        select(GitBackupLog)
        .where(GitBackupLog.config_id == config.id)
    ...
    if keep_last > 0:
        ...
        if keep_ids:
            delete_result = await db.execute(
                delete(GitBackupLog).where(
                    GitBackupLog.config_id == config.id, GitBackupLog.id.not_in(keep_ids)
                )
            )
        else:
            delete_result = await db.execute(delete(GitBackupLog).where(GitBackupLog.config_id == config.id))
    else:
        delete_result = await db.execute(delete(GitBackupLog).where(GitBackupLog.config_id == config.id))
