...
Matches the recent BamDude app log against a curated catalog of known failure
...
SIGNATURES: tuple[LogSignature, ...] = (
    ...
    LogSignature(
        ...
        # camera / LAN liveview is disabled. No logger_prefix: BamDude logs
        # camera errors from both services.camera ("Chamber image: ...") and
        # services.external_camera ("Camera connection test failed"), so a
        # single prefix would silently drop one source. The patterns are
        # distinctive enough to match safely without it.
