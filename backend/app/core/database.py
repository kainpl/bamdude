...
async def run_migrations(conn):
    ...
    # Migration: Add cleanup_after_print column to printers
    try:
        await conn.execute(text("ALTER TABLE printers ADD COLUMN cleanup_after_print BOOLEAN DEFAULT 1"))
    except OperationalError:
        pass  # Already applied

    # Migration: Add mqtt_connection_timeout column to printers
    try:
        await conn.execute(text("ALTER TABLE printers ADD COLUMN mqtt_connection_timeout INTEGER DEFAULT 300"))
    except OperationalError:
        pass  # Already applied
