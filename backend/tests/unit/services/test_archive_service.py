...
class TestThreeMFMetadataHTMLUnescape:
    ...
    BambuStudio sometimes writes triple-encoded payloads (the ProjectPageParser
    comment documents this). Without an unescape loop, a Title like ``Foo & Bar``
    lands in the DB as raw ``Foo &amp; Bar`` and React then escapes the ``&`` on
    render to ``Foo &amp;amp; Bar`` — the user-visible symptom on #1658."""
    ...
    def test_title_with_triple_encoded_ampersand_is_fully_unescaped(self, tmp_path):
        ...
        (`&amp;amp;amp;`). The decoder loops until the string stops changing so
        all layers get peeled in one pass."""
        import zipfile
