"""Tests for the FTPSession.cmd_STOR streaming + size-cap + ZIP-integrity behaviour.
...
peaked at ~2x the file size in RSS (chunks held + the ``b''.join`` of
...
On top of that, #1896 added a ZIP-integrity gate: a clean read-loop EOF does
not prove the upload arrived intact (uvloop can silently drop still-buffered
data on a ragged TLS close over slow storage), so a received ``.3mf`` must open
as a ZIP before we ack 226 — otherwise a truncated file gets archived, queued,
and forwarded to the real printer. These tests pin all three behaviours without
standing up a real TLS/FTP server.
"""
