...
export function PurgeOldFilesModal({ onClose }: PurgeOldFilesModalProps) {
  ...
  // Debounce the preview query so dragging the slider isn't a DoS.
  ...
  return (
    <Modal
      onClose={onClose}
      title={t('libraryPurge.title')}
      icon={<Trash2 className="w-5 h-5" />}
      size="lg"
      closeDisabled={purgeMutation.isPending}
    >
      <div className="p-4 space-y-4">
        <p className="text-sm text-bambu-gray">{t('libraryPurge.description')}</p>
