"""Per-slot filament colour on a slice request (upstream 4f10d155, #2977).
...
class TestTheRouteWiring:
    """Where the chain sits in ``_run_slicer_with_fallback``."""
    ...
    def test_colours_are_written_before_the_unused_slot_substitution(self):
        """The substitution copies slot 1's profile into unused slots, so it
        must copy a profile that already carries its colour."""
        source = self._source()
        patch = source.index("_patch_filament_colours(filament_jsons, request.filament_colours")
        substitute = source.index("substitute_unused_plate_filaments(primary_bytes")
        assert patch < substitute

    def test_they_are_written_after_every_slot_is_resolved(self):
        source = self._source()
        resolved = source.index('filament_jsons.append(await resolve_preset_ref(db, user, ref, "filament"))')
        patch = source.index("_patch_filament_colours(filament_jsons, request.filament_colours")
