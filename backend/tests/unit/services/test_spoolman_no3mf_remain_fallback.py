...
printer (subtask_name='名称未設定'/'Untitled'), BamDude creates a
fallback archive with no 3MF on disk (A6/A7, file_path=""). Before this
fix the Spoolman tracking row carried no usable data, so the print
silently didn't decrement the spool weight. This is the Spoolman mirror
of usage_tracker's Path 2 fallback (already in place for the internal-
inventory side).

BamDude adaptation vs upstream: ``ActivePrintSpoolman.filament_usage``
stays NOT NULL — store_print_data persists ``[]`` for the no-3MF case
(``filament_usage or []`` reads identically to None), so the migration is
a single nullable column add with no SQLite writable_schema hack.
"""
    ...
    async def test_creates_row_with_snapshot_when_no_3mf(self):
        ...
        with (
            ...
            await store_print_data(
                printer_id=1,
                archive_id=42,
                file_path="",  # fallback-archive file_path (A6/A7)
                db=db,
        ...
        # BamDude adaptation: [] not None (column stays NOT NULL).
        assert tracking.filament_usage == []
...
class TestReportUsageRemainDelta:
    ...
    async def test_remain_delta_writes_to_resolved_spool(self):
        ...
        tracking = SimpleNamespace(
            filament_usage=[],  # no-3MF: [] (BamDude keeps the column NOT NULL)
...
class TestPartialUsageRemainDelta:
    ...
    async def test_aborted_no_3mf_writes_remain_delta(self):
        ...
        # _report_partial_usage opens async_session() only for the two
        # get_setting reads (both mocked), so a mock db suffices.
        settings_db = AsyncMock()
        ...
        with (
            patch("backend.app.services.spoolman_tracking.async_session", lambda: _AsyncCtx(settings_db)),
            patch("backend.app.api.routes.settings.get_setting", AsyncMock(return_value="true")),
            patch(
                "backend.app.services.spoolman_tracking._get_spoolman_client_with_fallback",
                AsyncMock(return_value=client),
