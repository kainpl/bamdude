 ...
 * option when handed a value that is not in its list. Seeding it with a raw
 ...
 * dropdown and told the printer PLA-S.
 *
 * `tray_type` carries plenty of spellings the table does not list: support
 * materials (PLA-S) and composites (PETG-CF, PLA-CF, ABS-GF, PAHT-CF) all dry
 * as their base material, so the suffix is dropped before giving up.
 *
 * ⚠️ Anything still unrecognised lands on PLA, deliberately the COOLEST row.
 * Under-drying an exotic filament wastes a cycle; defaulting to PA's 85 °C
 * would deform a PLA spool.
 */
