...
//
// Extracted out of `SliceModal.tsx` so they can be unit-tested directly and so
// the modal component file only exports React components (the
...
//
// Selection rules:
// - Tier order is local → orca_cloud → cloud → standard. Local imports outrank
//   everything else because the user explicitly imported them for this install;
//   standard (bundled) is the final fallback.
// - The backend does NOT dedup tiers, so each helper walks all four and the
//   caller relies on the order, not a single merged list.
// - `pickProcessDefault` honours a 3MF's embedded process preset when it exists
//   and isn't printer-incompatible; otherwise prefers a match-on-printer pick,
//   then unknown-compat, then plain priority.
// - `pickFilamentForSlot` partitions candidates into compatible/unknown vs
//   mismatch buckets and only consults the mismatch bucket when the compatible
...
// Resolve a PresetRef back to its UnifiedPreset within the named slot, or null
// if it no longer resolves (e.g. the preset was deleted between the listing
// fetch and selection).
...
// Find a preset by exact name across tiers (local → cloud → standard). Used to
// honour the printer / process preset names a 3MF was prepared with.
...
export function pickFilamentForSlot(
  ...
  // similar-colour-match, and a small per-tier bonus breaks ties so cloud user
  // customisations win over standard bundled fallbacks of equal merit.
  //
  ...
  // (type, colour) happened to match it better than the colour-default standard
  // preset on the right printer — e.g. an unused slot whose embedded colour
  // matched `Generic PLA @BBL H2C` but not the off-the-shelf
  // `Bambu PLA Basic @BBL A1`. The propagated slot-1 then poisoned every unused
  // slot via `substitute_unused_plate_filaments`, and the CLI rejected the
  // slice with "filament preset Generic PLA @BBL H2C (slot 1) is not compatible
  // with printer Bambu Lab A1 0.4 nozzle". Hard-skipping mismatches while we
  // still have any compatible/unknown candidate eliminates that poisoning at the
