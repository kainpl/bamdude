...
# Runtimes that put BamDude in an OCI container whose network mode is a
...
# A string, turned into a Path when read: built at import it would be a
# WindowsPath on a Windows dev box, which no test patch of ``Path`` reaches.
_SYSTEMD_CONTAINER = "/run/systemd/container"
...
def detect_container_runtime() -> str | None:
    """Name the container runtime BamDude is running under, or None.
    ...
    """
    systemd_name = _read_text(Path(_SYSTEMD_CONTAINER)).strip().lower()
    if systemd_name:
...
# Virtual printer serial suffix to exclude from discovery (BamDude's own virtual printer)
...
class PrinterDiscoveryService:
    ...
    def _handle_response(self, response: str, ip_address: str):
        ...
        # Skip BamDude's own virtual printer (any model variant)
        if serial.endswith(VIRTUAL_PRINTER_SERIAL_SUFFIX):
            logger.debug("Ignoring BamDude virtual printer at %s", ip_address)
            return
...
class SubnetScanner:
    ...
    async def _probe_host(self, ip: str, timeout: float):
        ...
        if serial and serial.endswith(VIRTUAL_PRINTER_SERIAL_SUFFIX):
            logger.debug("Ignoring BamDude virtual printer at %s", ip)
            return
