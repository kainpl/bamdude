...
on ``bamdude.log`` returns the access line + every line emitted on the
...
# unlikely within a single rotation window of bamdude.log and grep stays
...
class TraceIDFilter(logging.Filter):
    ...
    Attached at the *handler* level rather than the root logger because
    Python's logging semantics only invoke a logger's filters on records
    that *originated* at that logger — records propagated up from child
    loggers (every named logger in the app) never trigger the root's
    filter. Putting the filter on each handler means every record any
    handler emits gets ``trace_id`` injected just before the formatter
    runs, regardless of which logger created the record. Without this,
    the formatter would raise ``KeyError`` on every child-logger record
    (and the stdlib logging machinery would silently drop it, leaving a
    log file that only carries records originating directly at root).
    """
