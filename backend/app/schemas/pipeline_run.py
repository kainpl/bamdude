"""Pydantic schemas for PipelineRun + eligibility (#1425, ported to BamDude)."""
...
class EligibilityIssueResponse(BaseModel):
    ...
    kind: Literal[
        ...
        "ams_slot_missing",
        "filament_unverified",
        "no_class_matches",  # target_kind='printer_class' and zero printers in the install match the model
...
class PerPrinterReport(BaseModel):
    ...
    Extends the top-level report with this list so the confirmation modal can
    show ``3 of 5 X1Cs eligible`` plus a per-printer breakdown of why each
...
class PipelineRunCreateRequest(BaseModel):
    """``copies`` defaults to 1. The route handler enforces the
    ``pipeline_max_copies`` setting on top of the schema's ``le`` bound."""
...
class PipelineRunResponse(BaseModel):
    ...
    status: Literal[
        ...
        "completed",
        "failed",
        "partial_failure",  # some copies succeeded, some failed/cancelled
        "cancelled",
    ]
...
class PipelineRunListResponse(BaseModel):
    ...
    total: int = 0  # for the dashboard's paginator
