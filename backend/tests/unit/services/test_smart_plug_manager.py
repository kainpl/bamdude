...
class TestSmartPlugManager:
    ...
    def mock_plug(self):
        ...
        # #1349 — drying defaults: off until the user opts in, so existing
        # tests don't accidentally activate the post-drying path. Mirrors
        # the column ``server_default="0"`` on m077.
    ...
    async def test_on_drying_complete_schedules_delayed_off_when_enabled(self, manager, mock_plug, mock_db):
        """Plug with ``auto_off_after_drying=True`` gets a delayed-off
        scheduled using its drying-specific delay (independent of the
        print-finish delay)."""
    ...
    async def test_on_drying_complete_skipped_when_toggle_off(self, manager, mock_plug, mock_db):
        """Default state — toggle off → nothing scheduled. Regression guard
        for users who only enable the print-finish auto-off and don't want
        the AMS-drying path silently running on the same plug."""
        mock_plug.auto_off_after_drying = False
        # ``auto_off`` itself is True (print-finish behaviour) — drying
        # path must still be a no-op without its own toggle.
    ...
    async def test_on_drying_complete_skipped_for_ha_script_entity(self, manager, mock_plug, mock_db):
        """HA ``script.*`` entities can be triggered but not turned off
        — same guard the print-finish path has."""
        mock_plug.auto_off_after_drying = True
    ...
    async def test_on_drying_complete_no_op_when_no_plugs(self, manager, mock_db):
        """Printer without any linked plugs is a silent no-op (not an
        error)."""
        with (
            patch.object(manager, "_get_plugs_for_printer", new_callable=AsyncMock) as mock_get_plug,
            patch.object(manager, "_schedule_delayed_off") as mock_schedule,
