/**
 * Unit tests for formatPrintName (utils/printName.ts).
 *
 * Regression coverage for upstream #881 follow-up: when the printer card has
 * an archive-linked plate label (resolved from the backend's current_archive_id
...
// Minimal translator stub that mirrors i18next's interpolation for the single
// key this helper consults. Keeps these tests independent of the i18n setup.
const t = (key: string, opts?: Record<string, unknown>) => {
  if (key === 'printers.plateNumber') return `Plate ${opts?.number ?? ''}`;
  return key;
};
