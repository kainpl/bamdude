...
- ``main.on_ams_change`` Spoolman tray sync (Spoolman RFID-driven sync)
...
async def upsert_slot_preset_for_spoolman_spool(
    *,
    ...
    The ``spoolman_spool`` dict is the shape returned by the Spoolman tray
    sync — ``spool["filament"]["name"]`` etc.
    """
