...
# relationship by string, and SQLAlchemy cannot resolve it unless the module
# has been imported. Without this the ORM-touching tests here pass only when
# something earlier in the run happens to have imported it first.
...
from backend.app.schemas.git_backup import GitRestoreRequest, RestoreCategory
from backend.app.services.git_restore import (
...
def _service() -> GitRestoreService:
...
class TestRestoreRequestSchema:
    ...
    def test_defaults_to_head(self):
        assert GitRestoreRequest(categories=[RestoreCategory.SPOOLS]).ref == "HEAD"

    @pytest.mark.parametrize("ref", ["HEAD", "abc1234", "a" * 40])
    def test_accepts_valid_refs(self, ref):
        assert GitRestoreRequest(ref=ref, categories=[RestoreCategory.SPOOLS]).ref == ref
    ...
    def test_rejects_refs_that_are_not_object_names(self, ref):
        with pytest.raises(ValueError):
            GitRestoreRequest(ref=ref, categories=[RestoreCategory.SPOOLS])
...
class TestRestoreKprofiles:
    ...
    async def test_sends_batch_to_connected_printer(self, db_session, printer_factory):
        ...
        with patch("backend.app.services.git_restore.printer_manager") as manager:
    ...
    # stored cali_idx 8151, a BamDude edit re-keyed the profile to 4606, and
    ...
    async def test_unknown_serial_is_skipped_with_reason(self, db_session):
        ...
        with patch("backend.app.services.git_restore.printer_manager"):
...
class TestSoftDeletedArchiveRoundTrip:
    ...
    async def test_deleted_at_survives_collect_then_restore(self, db_session):
        ...
        files: dict = {}
        await git_backup_service._collect_archives(db_session, files)
        payload = files[ARCHIVES_PATH]
        # ⚠️ isoformat(), not str(): every other datetime this collector writes
        # uses it, and upstream spelled this one field differently from its own
        # neighbours. The restore parses both, so the choice is consistency.
        assert payload["archives"][0]["deleted_at"] == deleted_at.isoformat()
...
class TestMutex:
    ...
    async def test_restore_refuses_while_a_backup_is_running(self):
        service = _service()
        with patch("backend.app.services.git_backup.git_backup_service") as backup:
    ...
    async def test_backup_refuses_while_a_restore_is_running(self):
        from backend.app.services.git_backup import GitBackupService

        backup_service = GitBackupService()
        with patch("backend.app.services.git_restore.git_restore_service") as restore:
...
class TestMqttRelayReconfigure:
    ...
    async def test_broker_failure_is_noted_not_fatal(self, db_session):
        ...
        assert any("restart BamDude" in note for note in _messages(tally))
...
class TestKprofilePhaseFailure:
    ...
    def _session_patch(self, db_session):
        ...
        return patch("backend.app.services.git_restore.async_session", return_value=cm)

    async def _configured_service(self, db_session, payload):
        from backend.app.models.git_backup import GitBackupConfig

        config = GitBackupConfig(repository_url="https://github.com/o/r", access_token="tok", provider="github")
...
class TestDescribeCommit:
    ...
    def _entry(self, sha: str):
        return {"sha": sha, "message": "BamDude backup", "author": "BamDude", "date": "2026-07-01T10:00:00Z"}
    ...
    async def test_a_commit_outside_the_window_is_fetched_directly(self):
        ...
        with patch("backend.app.services.git_restore.get_provider_backend", return_value=backend):
