...
- **Defects are recorded where you stand.** Every completed print card on an order page shows printed and defective and offers **Defects…**; on the printer card the finished print's counters sit beside **Clear plate** and **Repeat**, and either answer carries them. In Telegram each completion prompt names its printer, print and run, typed counts must reply to their own prompt, and clear/repeat buttons can affect only that named run — an old card cannot clear a newer plate. Statistics gain **Defects by printer**. A print already counted into free stock has its shelf credit corrected when defects are recorded later, in either direction.
