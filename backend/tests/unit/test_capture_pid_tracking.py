...
class TestCleanupExcludesCapturePids:
    ...
    @pytest.mark.skipif(
        ...
        reason="cleanup_orphaned_streams uses signal.SIGKILL; Windows has no such signal (and no /proc for the scan either, so the code path is Linux-only in production)",
    )
