...
# GitHub rate-limit backoff (#1420): when api.github.com returns 403/429 with
# X-RateLimit-Remaining=0, refuse to retry until X-RateLimit-Reset (epoch
# seconds). Falls back to a 1-hour pause if the header is absent. Prevents the
# update checker from hammering GitHub once the unauthenticated quota (60
# req/hr per source IP — easily shared/exhausted behind NAT) is spent.
_GITHUB_RATE_LIMIT_FALLBACK_SECONDS = 3600
_github_rate_limit_until: float = 0.0


def _seconds_until_github_unblocked() -> float:
    """Seconds remaining until the GitHub backoff lifts, or 0 if unblocked."""
    remaining = _github_rate_limit_until - time.time()
    return remaining if remaining > 0 else 0.0


def _record_github_rate_limit(response: httpx.Response) -> None:
    """Set the backoff window from a GitHub 403/429 response's headers."""
    ...
    # Floor at 60s: a reset epoch already in the past (container/GitHub clock
    # skew) would otherwise leave no real backoff and we'd hammer again at once.
...
# Mount points the shipped compose file gives BamDude. ⚠️ Only these are
# consulted when guessing the compose directory — an arbitrary bind mount (a NAS
# share, an external library root) says nothing about where the compose file
# lives.
_COMPOSE_BIND_MOUNTPOINTS = ("/app/data", "/app/logs")

# A named volume resolves to ``.../docker/volumes/<project>_bamdude_data/_data``
# in mountinfo. That names the compose PROJECT but reveals nothing about the
# directory holding the compose file, so these entries are skipped.
_DOCKER_NAMED_VOLUME_ROOT = re.compile(r"/docker/volumes/[^/]+/_data/?$")


def _compose_dir_from_mountinfo() -> str | None:
    """Guess the host directory holding the compose file, or None.

    ``docker compose pull`` only works from the directory containing the compose
    file, so the command the update box prints is unusable until the operator
    remembers where that is. Compose knows the answer — it stamps
    ``com.docker.compose.project.working_dir`` onto every container it creates —
    but reading your own labels requires the Docker socket, and ⚠️ mounting that
    into BamDude would hand the container root-equivalent access to the host in
    exchange for a convenience string. So we infer instead.

    ``/proc/self/mountinfo`` exposes the HOST side of a bind mount in its root
    field: a ``./data:/app/data`` line surfaces as ``/opt/bamdude/data``, whose
    parent is the compose directory. ⚠️ The leaf must match the mount point's own
    name before we take the parent — ``/mnt/nas/prints:/app/data`` is a bind
    mount whose parent is emphatically not a compose directory.

    ⚠️ This is a GUESS and is treated as one: it only ever prefills a field the
    operator can overwrite. The root field is relative to the mounted DEVICE
    rather than to the host's ``/``, so a compose directory sitting under a
    separate mount loses that mount's own prefix — and nothing inside the
    container can tell the two cases apart. The shipped compose file uses named
    volumes, for which nothing is inferable at all.
    """
    try:
...
def _detect_compose_dir() -> str | None:
    """Best-effort compose directory for the update instructions.

    ``BAMDUDE_COMPOSE_DIR`` wins when set — it is the only source that is
    STATED rather than inferred, and the shipped compose file carries a
    commented ``${PWD}`` line for it.
    """
    env_dir = os.environ.get("BAMDUDE_COMPOSE_DIR", "").strip()
    if env_dir:
        return env_dir
...
async def _find_latest_release(include_beta: bool) -> dict | None:
    ...
    """
    # Don't even open the socket while a GitHub rate-limit backoff is active
    # (#1420). Shared by /check and /apply, so this covers both call sites.
    if _seconds_until_github_unblocked() > 0:
        logger.warning("Skipping GitHub release lookup: rate-limit backoff still active")
        return None
...
async def check_for_updates(
    ...
    try:
        ...
        if not release_data:
            # Distinguish a genuine "no releases" from a GitHub rate-limit
            # backoff (#1420) so the UI can show a real wait time instead of an
            # opaque failure.
        ...
        return {
            ...
            # ⚠️ Prefill only — never the value the operator saved. The settings
            # response owns ``docker_compose_dir``; keeping the two apart means
            # clearing the field falls back to the guess instead of resurrecting
            # the cleared value from a stale update check.
